...
**Epic:** Slab 2c (migration Epic 2c — Wondercraft Pilot + Generator Validation) — **third story; M2 acceptance gate**.
**Pts:** 1 | **Gate:** single (per governance JSON `2c-3.expected_gate_mode = "single-gate"`, rationale: null — measurement + roll-up + 5-agent party-mode validation; no schema-shape, no lane-boundary, no invariant-preservation; the M2 verdict-recording artifact). **K-target:** ~1.2× (target 6 / floor 5; documentation-heavy + measurement-roll-up + party-mode evidence; minimal new test surface).
...
- **A-R3-2c.3 BLOCKER B3 (`tests/specialists/slab_2c/` violates convention) RESOLVED:** Test paths moved from `tests/specialists/slab_2c/test_m2_*.py` → `tests/migration/test_slab_2c_m2_*.py` (4 flat files) per existing `tests/migration/test_bmb_scaffold.py` precedent. NO `slab_2c/` subdirectory.
...
- **A-R5-2c.3 (5-voice viability):** T1 sub-task added — verify each named agent has voice on M2-acceptance question. If Quinn-R abstains ("outside lane"), 4-of-5 consensus path is acceptable; not a defect.
- **A-R6-2c.3 (conditional-green outliving 2c.4 close):** AC-D-PARTIAL extended with hard-gate clause — if 2c.4 reaches dev-close before operator-window completes, Slab 2c closes as `CLOSED-WITH-CONDITIONAL-M2`, NOT `CLOSED-GREEN`. M2 milestone remains `CONDITIONAL-GREEN-PENDING-OPERATOR-ADDENDUM` until operator pastes addendum.
- **M-R1-2c.3 (AC-A math assertion strengthen):** AC-A test extended — assert (a) cumulative-active-hours numeric value matches sum of STRUCTURED TIMER deltas within ±0.1h tolerance, AND (b) verdict-band line in report matches the band the cumulative-hours value falls into per AC-A GREEN/YELLOW/RED thresholds. K-floor +1 firm.
- **M-R2-2c.3 (AC-C verdict-token enum):** AC-C test sharpened — replace ≥50-char body check with: each agent sub-header section contains exactly one line matching `^Verdict: (GREEN-LIGHT|GREEN-WITH-RIDERS|CONDITIONAL-GREEN|YELLOW|RED|ABSTAIN)$` AND section body ≥150 chars excluding that line.
...
- **M-R4-2c.3 (T_first_real_artifact semantic pin):** Decision #2 footnote — `T_first_real_artifact ≡ wall-clock instant LIVE_ARTIFACT_METADATA.md is written-to-disk with all required schema fields populated, after artifact-assembly returns success exit code. Operator-validation is a downstream gate, not included in time-to-deploy budget.`
- **M-R5-2c.3 (K-floor disclosure rewording):** Reframed honestly — "5 firm + 1 conditional-rider-absorbed-via-AC-C-verdict-line = 6, exactly target. No padding."
...
- **Soft fallback:** if 2c.2 AC-D-OP is `DEFERRED-PENDING-OPERATOR-WINDOW` at 2c.3 open, 2c.3 may still proceed with **PARTIAL-VERDICT** at AC-D (see AC-D-PARTIAL clause below); M2 verdict transitions GREEN-LIGHT → CONDITIONAL-GREEN with `2c-3-m2-verdict-conditional-on-2c-2-live-artifact` deferred-inventory entry.
...
1. **Governance lookup** — `docs/dev-guide/migration-story-governance.json` confirms `2c-3.expected_gate_mode = "single-gate"` (rationale: null). Do not relitigate.
2. **Predecessor close evidence** — Stories 2c.1 + 2c.2 expected `done` per sprint-status.yaml. Verify via `grep "2c-1.*done\|2c-2.*done" sprint-status.yaml` before proceeding. If either is `in-progress` or `ready-for-dev`, abort 2c.3 and surface to operator.
3. **2c.1 diff-evidence anchor** — `_bmad-output/implementation-artifacts/2c-1-wondercraft-path-b-diff-evidence.md` (per 2c.1 AC-G PERSISTENT contract). Verify present + structurally valid (parses as Markdown + has expected sections per 2c.1 AC-C two-tier diff format).
4. **2c.2 live-artifact anchor** — `tests/fixtures/specialists/wanda/live_artifacts/2026-04-26/<sha256>.mp3` + `LIVE_ARTIFACT_METADATA.md` companion (per 2c.2 AC-D-OP). Verify present OR confirm DEFERRED-PENDING-OPERATOR-WINDOW state from 2c.2 close notes (triggers AC-D-PARTIAL).
5. **2c.1 + 2c.2 timer evidence** — Dev Agent Records of 2c.1 (T8 STRUCTURED TIMER TABLE per Murat M-R3) + 2c.2 (T8 measurement evidence). Verify the timestamp tables are populated.
6. **TEMPLATE doc** — [`docs/dev-guide/specialist-migration-template.md`](../../docs/dev-guide/specialist-migration-template.md) v2.4 R1–R14. Most rules N/A (no migration in scope; this is measurement + verdict). Applicable: R1 bounded scope; R6 framework drift (none expected); R8 K-floor recalibration.
7. **Party-mode roster pin** — Epic 2c.3 binding requires Winston + Murat + Paige + Quinn-R + Amelia (5 voices). Roster is HARD; do NOT substitute.
...
**Decision #1 — Bounded scope (per R1):** scope is (a) time-to-deploy roll-up across 2c.1 + 2c.2 timer evidence; (b) 5-agent party-mode convene + verdict recording; (c) 15-invariant audit matrix Wondercraft entries (FR63 incremental roll-up); (d) M2 verdict artifact at `_bmad-output/implementation-artifacts/slab-2c-m2-acceptance-verdict.md`. NOT in scope: re-running 2c.1 or 2c.2 measurements; modifying Wanda runtime; producing additional live artifacts; re-litigating Path A vs Path B decision.

**Decision #2 — Time-to-deploy roll-up methodology:** roll-up ANCHORS on **2c.1 T0 → 2c.2 first-real-artifact** end-to-end window. The 2c.1 T0 timestamp is the dev-story-open anchor (per 2c.1 AC-E); 2c.2 first-real-artifact is the operator-paste timestamp (per 2c.2 AC-D-OP LIVE_ARTIFACT_METADATA.md `end_timestamp`). Active dev-work hours computed by SUMMING active intervals from 2c.1 + 2c.2 STRUCTURED TIMER tables (excluding paused intervals per Murat M-R3 sub-table convention). Per epic 2c.3 binding: ≤1 dev-day = ≤8 active clock hours.

> **Semantic pin per Murat M-R4-2c.3 (binding):** `T_first_real_artifact ≡ wall-clock instant LIVE_ARTIFACT_METADATA.md is written-to-disk with all required schema fields populated, after artifact-assembly returns success exit code.` Operator-validation (e.g., listening to MP3, judging quality) is a downstream gate and is **NOT** included in the time-to-deploy budget. This pin prevents M2 re-litigation 6 months later when "what counted as first-real-artifact?" surfaces.

**Decision #3 — PARTIAL-VERDICT path if 2c.2 AC-D-OP defers:** if 2c.2 closed with AC-D-OP DEFERRED, 2c.3 cannot fully measure end-to-end time-to-deploy. **PARTIAL-VERDICT clause activates at AC-D-PARTIAL:** measure 2c.1 T0 → 2c.2 dev-story-close; party-mode verdict transitions to CONDITIONAL-GREEN with the 2c.2 deferred operator-window as the conditional resolution gate. M2 closes "conditionally" at 2c.3 close; FULL M2 closure pending operator-window completion + addendum at `slab-2c-m2-acceptance-verdict.md` §"Operator-Window Addendum."

**Decision #4 — Party-mode artifact persistence:** 5-agent party-mode verdict + each agent's individual response recorded VERBATIM in `slab-2c-m2-acceptance-verdict.md`. Do NOT summarize — full verbatim per agent so the verdict is traceable and any future M2 retrospection can audit the consensus path.
...
All ACs are dev-agent-executable except AC-2c.3-C (party-mode convene with 5 named agents — runs through bmad-party-mode skill). Sandbox-AC compliant.

### AC-2c.3-A — Time-to-deploy roll-up table assembled

- **Given** 2c.1 + 2c.2 Dev Agent Record T8 STRUCTURED TIMER tables are populated (per 2c.1 M-R3 + 2c.2 M-R3 inheritance)
- **When** the dev agent assembles the roll-up table at `_bmad-output/implementation-artifacts/slab-2c-m2-acceptance-verdict.md` §"Time-to-Deploy Measurement"
- **Then** the table includes:
  ...
  | T_2c1_emit | 2c.1 Dev Agent Record T8 | (paste; ≤30 sec from T0) | (computed) |
  | T_2c1_conformance_green | 2c.1 Dev Agent Record T8 | (paste; ≤5 min from T0) | (computed) |
...
- **And** measurement compared against epic 2c.3 binding: **≤1 dev-day = ≤8 active clock hours**. Verdict per (4-enum band per Murat M-R3 audit):
  - **GREEN-LIGHT:** ≤6 hours (well within budget)
  - **YELLOW:** 6-8 hours (within budget; tight)
  - **RED:** >8 hours (over budget; root-cause documented in §"Time-to-Deploy Overrun RCA" + filed as `2c-3-time-to-deploy-overrun-rca` follow-on)
...
- **Test pin per Murat M-R1-2c.3 (strengthened from structural smoke to math-asserting):** `tests/migration/test_slab_2c_m2_verdict_artifact_present.py` — 1 test asserting:
  1. `_bmad-output/implementation-artifacts/slab-2c-m2-acceptance-verdict.md` exists.
  2. §"Time-to-Deploy Measurement" section header present.
  3. Filled-in row count ≥ 12 in the time-to-deploy table (or ≥ 11 + `T_first_real_artifact` row marked `DEFERRED-PENDING-OPERATOR-WINDOW` if AC-D-PARTIAL active).
  4. **MATH ASSERTION (M-R1):** parse the table; sum the per-row active-hour deltas (excluding rows with paused-interval markers); assert the cumulative-hours value at the bottom of the table is within ±0.1h tolerance of the computed sum.
  5. **VERDICT-BAND ASSERTION (M-R1):** parse the verdict-band line at top of §"M2 Required Evidence Summary"; assert it matches the band the cumulative-hours value falls into per the GREEN-LIGHT/YELLOW/RED/CONDITIONAL-GREEN thresholds (4-enum regex per M-R3 audit).

### AC-2c.3-B — Time-to-deploy verdict line + cost-summary line

- **Given** AC-A roll-up table assembled
- **When** dev agent appends to `slab-2c-m2-acceptance-verdict.md` §"M2 Required Evidence Summary":
  - **Time-to-deploy verdict:** GREEN / YELLOW / RED per AC-A bands; total active-clock-hours numeric value
  - **Cost summary:** 2c.1 AC-B-OP smoke-test ($0.00 to $0.50 actual) + 2c.2 AC-D-OP production-quality ($X.XX actual per Wondercraft response, scripted OR simple-fallback per A-R4-2c.2 enum); cumulative cost ≤ $5.50 if scripted-path; ≤ $2.50 if simple-fallback-path
  - **Diff-evidence verdict:** PASS (≥60% file-presence + ≥40% skeleton-line per 2c.1 AC-C two-tier) / FAIL with score values
...
- **Then** each agent's response is recorded **verbatim** in `slab-2c-m2-acceptance-verdict.md` §"Party-Mode Verdict (5 agents)" under sub-headers `### Winston`, `### Murat`, `### Paige`, `### Quinn-R`, `### Amelia` per Decision #4. **Do NOT summarize.**
- **And** consensus verdict recorded at top of §"Party-Mode Verdict" matching `^Consensus verdict: (GREEN-LIGHT|GREEN-WITH-RIDERS|CONDITIONAL-GREEN|YELLOW|RED)$` (4-enum decision band per Murat M-R3 audit; ABSTAIN is per-agent only, not a consensus-level verdict) with vote breakdown per agent.
- **And** any agent-named riders aggregated under §"Riders" with `<agent>-R<n>-2c.3` notation; each rider classified as (a) APPLIES-TO-2c.3-CLOSE (resolve before close), (b) APPLIES-TO-2c.4 (defer to slab-close story), (c) DEFERRED-INVENTORY (file as follow-on).
- **Test pin per Murat M-R2-2c.3 (sharpened from ≥50-char weak check to verdict-token-enum + 150-char body):** `tests/migration/test_slab_2c_m2_party_mode_5_agent_recording.py` — 1 test asserting:
  1. §"Party-Mode Verdict (5 agents)" section exists.
  ...
  4. **BODY-LENGTH ASSERTION (M-R2):** each sub-header section body ≥150 chars EXCLUDING the verdict-token line (deterministic; no LLM-output semantic-content assertion).
  5. Consensus verdict line at top of §"Party-Mode Verdict" matches `^Consensus verdict: (GREEN-LIGHT|GREEN-WITH-RIDERS|CONDITIONAL-GREEN|YELLOW|RED)$` (4-enum consensus-level).

### AC-2c.3-D — Wondercraft invariant-stub artifact for Slab 5a absorption (NARROWED per A-R1 BLOCKER B1)
...
- **Then** the file contains 2 row-stubs (one per Wondercraft specialist) with the metadata Slab 5a will need to absorb at matrix creation time:
  - **`wanda`** (production specialist; Path A; closed 2b.8 + 2c.2): specialist-name, slab-of-origin (`2b.8`), shape-category (`§12.11 REST-API tool-dispatch inheritor`), expertise-tier (`L4-podcast-production` extended to L5 at 2c.2), sanctum-status (`populated 2c.2`), live-API-status (`exercised 2c.2 AC-D-OP` OR `DEFERRED-PENDING-OPERATOR-WINDOW`), evidence-anchor (`2c.2 LIVE_ARTIFACT_METADATA.md`)
  - **`wanda_validation`** (transient validation tree; Path B; closed 2c.1 then retired): specialist-name, slab-of-origin (`2c.1`), shape-category (`§12.11 REST-API tool-dispatch via generator`), expertise-tier (`L5-podcast-production stub`), retirement-disposition (`retired-to-fixtures at tests/fixtures/generator_validation/wanda_baseline/2026-04-25/`), evidence-anchor (`2c-1-wondercraft-path-b-diff-evidence.md`)
- **And** the file headers note "STUB FOR SLAB 5A INVARIANT AUDIT EPIC ABSORPTION — created 2c.3 to capture Wondercraft-specific facts at M2 close before Slab 5a matrix creation."
- **Test pin:** `tests/migration/test_slab_2c_m2_wondercraft_invariant_stub.py` — 1 test asserting `_bmad-output/implementation-artifacts/slab-2c-wondercraft-invariant-stub.md` exists + contains rows for both `wanda` and `wanda_validation` + each row has the 6-7 metadata fields populated above.

### AC-2c.3-D-PARTIAL — Conditional-green path if 2c.2 AC-D-OP defers
...
- **Then**:
  ...
  - AC-C consensus verdict transitions GREEN-LIGHT → CONDITIONAL-GREEN
  - **Auto-file follow-on:** `2c-3-m2-verdict-conditional-on-2c-2-live-artifact` in deferred-inventory; reactivation gate "operator completes 2c.2 AC-D-OP and pastes addendum to slab-2c-m2-acceptance-verdict.md §Operator-Window Addendum"
  - **Hard-gate clause per A-R6-2c.3 (BINDING):** if Slab 2c.4 SLAB-CLOSING reaches dev-close before operator-window completes, Slab 2c closes as `CLOSED-WITH-CONDITIONAL-M2`, **NOT** `CLOSED-GREEN`. M2 milestone remains `CONDITIONAL-GREEN-PENDING-OPERATOR-ADDENDUM` until operator pastes addendum to `slab-2c-m2-acceptance-verdict.md` §"Operator-Window Addendum" + dev runs `2c-3-m2-verdict-conditional-on-2c-2-live-artifact` reactivation cleanup. Slab 2 (parent) closes as `CLOSED-WITH-CONDITIONAL-M2` if Slab 2c carries that state through close.
...
- **Test pin:** None new — AC-D-PARTIAL is a conditional branch of AC-A through AC-C ACs; the AC-A and AC-C verdict-band/verdict-token regexes BOTH whitelist `CONDITIONAL-GREEN` per M-R3 4-enum audit (verified inline in AC-A point 5 + AC-C point 5).

### AC-2c.3-E — Anti-pattern catalog harvest (per R6)

- **Given** the catalog is final-form post-2b.17 with A1–A13
...
### AC-2c.3-F — TEMPLATE compliance (per R1–R14 v2.4)

R1–R14 v2.4 honored where applicable. **Most rules N/A** (no migration; this is measurement + verdict recording). Applicable: R1 bounded scope (Decision #1); R6 framework-drift harvest (none expected at this story per (a) above); R8 K-floor recalibration (~1.2× = target 6 / floor 5 honored).

### AC-2c.3-G — D12 close protocol (single-gate; FOUR-line per A-R7)
...
### AC-2c.3-H — Sprint-status state-flips at filing AND at close
...
├── slab-2c-m2-acceptance-verdict.md             # M2 verdict-recording artifact (NEW per AC-A/B/C)
└── slab-2c-wondercraft-invariant-stub.md        # 2-row stub for Slab 5a absorption (NEW per AC-D)

tests/migration/                                  # FLAT layout per Amelia A-R3 BLOCKER B3 (no slab_2c/ subdir)
├── test_slab_2c_m2_verdict_artifact_present.py            # 1 test (AC-A; math + verdict-band assertions per M-R1)
├── test_slab_2c_m2_required_evidence_section_complete.py  # 1 test (AC-B; 4-line regex + 4-enum verdict)
├── test_slab_2c_m2_party_mode_5_agent_recording.py        # 1 test (AC-C; verdict-token enum + 150-char body per M-R2)
└── test_slab_2c_m2_wondercraft_invariant_stub.py          # 1 test (AC-D; 2-row stub presence + metadata fields)
```

### MODIFIED files

- `_bmad-output/implementation-artifacts/sprint-status.yaml` — per AC-H.
- `_bmad-output/planning-artifacts/deferred-inventory.md` — `2c-3-time-to-deploy-overrun-rca` filed if AC-A verdict RED. `2c-3-m2-verdict-conditional-on-2c-2-live-artifact` auto-filed if AC-D-PARTIAL fires.
...
- Wanda runtime (`app/specialists/wanda/`) — DO NOT TOUCH (2c.2 close pin).
- 2c.1 diff-evidence Markdown — READ-ONLY input (per 2c.1 AC-G PERSISTENT contract).
- 2c.2 live artifact + LIVE_ARTIFACT_METADATA.md — READ-ONLY input.
- Generator (`skills/bmad_create_specialist/`) — DO NOT TOUCH (2c.4 polish-scope).
- All 14 other specialist runtimes — out of scope.

---

## Testing Requirements

**K-target ~1.2× (target 6 / floor 5).** Test count and K-floor accounting per Murat M-R18 honest-count + M-R5-2c.3 rewording:
...
**Regression target at T8:** ≥562 passed / ≥7 skipped placeholder-key (Slab 2b close baseline preserved); +4 collected at file level. Import-linter 3/3 KEPT throughout. Ruff clean. Sandbox-AC PASS (no live API calls in 2c.3 dev-agent ACs; reads from 2c.1 + 2c.2 evidence files only).
...
_(Populated at T8 with full timer roll-up + 5-agent party-mode prompt + each agent's verbatim response + consensus verdict)_
...
_(Populated at story close per AC-2c.3-G)_
