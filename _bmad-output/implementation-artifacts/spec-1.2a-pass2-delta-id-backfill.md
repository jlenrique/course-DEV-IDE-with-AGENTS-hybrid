...
## ⚠️ Diagnosis sharpened on real data (2026-06-24, post-first-commit)
The first commit (`673f1b6`) assumed the deltas were fully id-LESS and backfilled positionally from narration. Inspecting the REAL checkpoint (`52890be7`) showed the deltas are **not** id-less — they carry the id under **`segment_id`** (seg-01…13, correctly matching narration) plus a direct `slide_id`, but **no `id`**. The party-governed `join_narration_segments` keys on `delta.get("id")` ONLY (ignoring `segment_id`), while the sibling `_segment_slide_id_from_deltas` reads BOTH — a key inconsistency. Empirical: current join drops 13/13; after `segment_id`→`id` alias, **0 dropped, 13 non-empty texts**. So the real fix is the **`segment_id`→`id` alias** (Step 1, direct, always safe, per-delta); the original positional-from-narration backfill is retained as a **gated defensive fallback** (Step 2, all-None + equal-length only). Positional backfill was a no-op on the real bug.
