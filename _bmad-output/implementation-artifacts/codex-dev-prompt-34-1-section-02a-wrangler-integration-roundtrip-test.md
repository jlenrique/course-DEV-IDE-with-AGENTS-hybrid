...
- **Story-34-1 SCOPE BOUNDARY post-Codex-T1-surface amendment 2026-05-22 (BINDING):** the test MUST NOT assert `metadata.json::sme_refs[]` shape. Story 34-4 owns the wrangler's sme_refs[] emission; a translator that only changes directive INPUT shape cannot make the wrangler emit a new metadata.json OUTPUT key. Codex correctly halted on this contradiction at T1; AC-34-1-A is now amended to omit the sme_refs assertion. Story 34-4 will EXTEND this test to add the sme_refs assertion block in lockstep with the new wrangler behavior (per AC-34-4-A-EXT). If you see "sme_refs" referenced in older spec snapshots — that's the pre-amendment text; current contract excludes it from Story 34-1.
