...
**Status:** **S1 + S2 + S3 ALL DONE** (2026-06-23 — each: Codex NEW CYCLE → Claude T11 → party 5/5 CLOSE; S1/S2 each one hand-back, S3 CLOSE-WITH-RIDERS). **The reading-path tuple classifier is fully BUILT but UNCALIBRATED.** ⚠️ The S3-T11 live dry-run found the built classifier scores ~0.071 primary-key on stale held-out perceptions (the 0.93 was the catalog-approach, not the classifier) + escalation over-fires 93%. **P2-4b is now a real 4-leg calibration milestone** (re-perceive under role_tier → recalibrate → measure → contingent improve; `spec-p2-4b-conformance-finalize.md §4`), NOT a rubber-stamp.
...
- **S2 EMISSION CONTRACT (CLOSED 2026-06-23, party 5/5 — canonical; do NOT "clean up"):** `image_roles` is a **FULL-LENGTH array index-aligned 1:1 with `visual_elements`**, with **`None` as a positional sentinel for unscored / bbox-less elements** (NOT a compacted list — silent compaction was MF-A). **`image_roles[i] == None` means UNSCORED; consumers (S3, runner, scoring fold, Irene, P2-4b) must skip/None-guard it and NEVER coerce it to a default tier.** Invalid perceiver `role_tier` values backfill + raise the `dropped_invalid_tier` side-channel flag (SF-2; no silent drop). Both additions are additive (schema widened for `None` + the flag; nothing removed).
