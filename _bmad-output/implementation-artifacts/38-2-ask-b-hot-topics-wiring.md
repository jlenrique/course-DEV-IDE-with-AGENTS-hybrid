...
| AC 6 (rows 7, 9, 13, 14) | `test_ask_default_factory_upgrades_stub_and_retryable_is_exact_noop[07W.2/07W.4]` (row 7; M-4 posture: factories=None, dispatch monkeypatched only at the owned seam) · `test_reconcile_not_skip_set_is_exactly_the_activated_band_tail` (W-2 set EQUALITY) · `test_ask_b_completed_contribution_resume_is_exact_noop` (row 9) · **`test_ask_conflicting_completed_contribution_fails_split_brain[ask-a/ask-b]` (row 14 — completed journal + CONFLICTING completed contribution; T4 review R2, Auditor row-14 fix: the prior entry mis-cited the without-journal test)** · `test_ask_completed_envelope_without_journal_fails_split_brain[ask-a/ask-b]` (completed-contribution-without-journal split-brain, AC 6 reconciliation table) · marker identity + typed-retryable output pins inside `test_default_band_executes_real_brief_and_typed_ask_retryables` · journal-backed roll-forward covered by `test_one_dispatch_completed_journal_and_zero_call_replay` + row-9 test (row 13) |
...
## Senior Developer Review (AI) — T4 remediation record (2026-07-16)

T4 review at baseline `9fb95f9f` (the 38-2 feature commit) surfaced 13 findings; all 13 are RESOLVED in this remediation diff, each with a named test citing its finding id. **All Ask-B contract shapes were PRE-WITNESS at remediation time** (the probe has never run; no frozen Ask-B artifacts exist anywhere), so the contract-shape changes below were safe and are declared in the Change Log. Fences held: `production_runner.py`, `state/config/pipeline-manifest.yaml`, `trends_projection.py` — zero lines changed (git-verified).

| Finding | Severity | Resolution | Named test(s) |
|---|---|---|---|
| R1a (B1/E4/E5/E2, live-repro'd) | HIGH | ALL fallible construction (scope build, `_build_intent`) moved BEFORE the lock claim and pre-call journal write; init failures typed `ask-b.dispatch-init-failed`; a stranded `call_in_progress` from init is now impossible | `test_intent_init_failure_is_typed_and_never_strands_call_in_progress` · `test_scope_build_failure_is_typed_dispatch_init_failed` |
| R1b (B1/E4) | HIGH | Per-row loss coverage exhaustive: entry construction wrapped → indexed `ask_b_row_entry_invalid:{index}` + continue; prechecks aligned with validators (`_clean_line` rejects ANY line-control char + strip-inequality in provider/source_id; excerpt checked non-blank AFTER the 2000-char window slice) | `test_row_entry_validator_failure_is_indexed_loss_after_spend` · `test_control_char_provider_or_source_id_is_indexed_source_loss` · `test_blank_after_window_excerpt_is_indexed_evidence_loss` |
| R1c (E5) | HIGH | Citation mint survives >999 rows: `f"{n:03d}"` widens naturally; entry regex flipped to `ask-b-cite-[0-9]{3,}` (pre-witness shape change) | `test_thousand_row_mint_keeps_contiguous_natural_width_citations` · `test_citation_namespace_grows_naturally_past_999` |
| R1d (E1) | HIGH | `_read_journal` catches `UnicodeDecodeError` → `ask-b.reconciliation-failed` | `test_journal_unicode_decode_error_is_reconciliation_failed` |
| R2 (Auditor row-14) | MUST | Conflicting-completed split-brain branch (workbook_wiring ~L2067) has its own parametrized proof per the M-7 idiom; the row-14 traceability entry above now names it (prior entry mis-cited the without-journal test) | `test_ask_conflicting_completed_contribution_fails_split_brain[ask-a/ask-b]` |
| R3 (B2, live-repro'd) | MED | Stub-detection gate keys on FULL stub-shape equality (`_ASK_A_LEGACY_STUB_OUTPUT` / `_ASK_B_LEGACY_STUB_OUTPUT` constants) at BOTH band coordinates — the Ask-A flank fixed as a declared conscious symmetric hardening (its gate mirrored the same flaw); forged completed-claiming contribution carrying the magic key → strict validation → `reconciliation-failed`, never a re-dispatch | `test_forged_completed_contribution_with_stub_magic_key_never_redispatches[ask-a/ask-b]` |
| R4a (E8) | MED | Output validator rejects forged associations: every entry's `supports_ability_ids` ⊆ scope ability ids | `test_forged_association_outside_scope_is_rejected_in_output` |
| R4b (B9) | MED | `load_research_packet` rejects DUPLICATE contributions at the exact strict Ask-B coordinate (mirrors the demand resolver's coordinate-collision paranoia) | `test_duplicate_ask_b_contributions_at_coordinate_fail_loud` |
| R5a (B6/E7) | MED | `AskBHotTopicsDemandV1` rejects `\r`/`\n`/U+2028/U+2029 in vow text → typed `ask-b.demand-invalid` at the seam; trailing whitespace stays contract-legal | `test_constructed_ready_demand_rejects_line_control_vow_text[×3]` · `test_constructed_ready_demand_accepts_trailing_space_vow_text` |
| R5b (B7, injection repro'd) | MED | Canonical query = derived display: vow/scene text projected through `_query_display_text` (delimiter `\| ; [ ]` neutralization + whitespace collapse, symmetric with scene); structured fields remain identity authority; query digest still binds | `test_query_neutralizes_scope_delimiters_from_vow_and_scene_text` |
| R5c (B6, repro'd) | MED | Trailing-space vow (contract-legal upstream, PromiseVow NonBlankStr) resolves to a working scope — never crashes the single-line query contract | `test_trailing_space_vow_resolves_to_working_scope` |
| R6 (B8) | LOW/contract | `matched_ability_tokens` → ORDERED list of `{ability_id, tokens}` pairs (`AskBAbilityTokenMatchV1`); a `sort_keys` rewrite cannot flip validity; validator equality check updated; digest covers the list (pre-witness, safe) | `test_association_evidence_is_ordered_pairs_not_object_key_order` |
| R7 (B3) | MED | `query_limit` removed from fingerprint/scope-digest/idempotency; limit parsed + enforced pre-dispatch ONLY; limit VALUE journaled for observability (non-identity); non-integer/`<1` env → NEW typed tag `ask-b.config-invalid` (never scope-overflow) | `test_query_limit_is_operational_never_identity` (completed journal replays green after env change, incl. garbage) · `test_non_integer_query_limit_is_typed_config_invalid` |
| R8 (B10) | MED | `provider_config_fingerprint` made real: digests the ACTUAL dispatch-shaping inputs (provider name/mode, static adapter qualname, iteration budget, acceptance-criteria constants) SHARED with `_build_intent`, so provider drift changes it; adapter qualname is a static constant so an M-4 injected test double never flips replay identity | `test_provider_config_fingerprint_binds_dispatch_shaping_inputs` |
| R9 (B5) | MED | Shared `load_run_envelope` (workbook_enrichment.py) rejects symlinked `run.json` → `RunEnvelopeCorruptError` (additive containment guard) | `test_symlinked_run_json_is_corruption_not_honest_absence` (skips on hosts without symlink privilege, per the existing idiom) |
| R10 (B4) | MED | Pending-witness state machine: if any probe evidence pack dir exists (`ask-b-38-2-live-*` — the pattern `run_ask_b_38_2_live_evidence.py` writes — or `ask-b-hot-topics-38-2*`), a still-`pending` disposition FAILS the suite (flip machine-forced); registry pending row carries top-level `owed_by_probe` naming the probe, machine-asserted | `test_pending_flip_is_machine_forced_once_probe_evidence_exists` (fail-path self-verified against a scratch pack dir) · `test_pending_rows_name_their_owing_probe` |
| R11 (E6) | LOW | Ability with zero association-basis tokens → per-ability `ability_association_basis_absent:<ability_id>` scope loss, recomputed by the validator, carried into packet `known_losses`; dispatch continues (visible, never silent) | `test_acronym_only_ability_records_association_basis_loss` ("AI"-only vow) |
| R12 (E9) | LOW | Replay divergence check now compares journal `research_intake` against the replayed truth | `test_replay_divergence_check_includes_research_intake` |
| R13 (B11) | LOW | `_fsync_dir` platform-guarded helper (POSIX: real directory fsync; Windows: documented no-op — `os.open` of a dir fails there) invoked after lock create + after every journal `os.replace` | `test_parent_directory_fsync_intent_covers_lock_and_journal_writes` |

**Findings skipped: NONE.** Battery (post-remediation): Command A (`tests/unit/marcus/lesson_plan` + `tests/unit/marcus/orchestrator` + `tests/integration/marcus/test_workbook_band_wiring.py`) → `1 failed, 1677 passed, 5 skipped` — the single failure is the identical inherited baseline signature (`test_health_tiles_prefer_persisted_cost_report`, `KeyError: 'run cost'`; pre-dates 38-2); the +1 skip is the new R9 symlink test on this no-symlink-privilege host. `WITNESS_REPLAY_STRICT=1 tests/live_witness_replay -n 0` → `27 passed, 0 skipped` (R10 pins included; fail-path proven with a scratch evidence dir, then removed). `tests/marcus/lesson_plan/test_run_envelope_corrupt_fail_loud.py` → 4 passed (shared R9 guard non-regression). Ruff clean on every touched file.
...
- `app/marcus/lesson_plan/workbook_enrichment.py` (T4 R9: shared `load_run_envelope` symlink guard)
...
- 2026-07-16 — **T4 review remediation (Claude Fable 5, remediation dev): all 13 findings R1–R13 resolved** (per-finding table in §Senior Developer Review (AI)). **PRE-WITNESS contract changes, declared safe because the probe has never run and no frozen Ask-B artifacts exist anywhere:** (1) entry citation regex widened to `ask-b-cite-[0-9]{3,}` (R1c); (2) `matched_ability_tokens` reshaped from a JSON object to an ORDERED list of `{ability_id, tokens}` pairs via new `AskBAbilityTokenMatchV1` (R6); (3) scope `known_scope_losses` lattice extended with per-ability `ability_association_basis_absent:<id>` losses, validator-recomputed (R11); (4) `provider_config_fingerprint` re-derived from the real dispatch-shaping inputs and `query_limit` REMOVED from all identity surfaces (fingerprint/scope digest/idempotency key) — limit journaled as observability only (R7+R8); (5) NEW stable tag `ask-b.config-invalid` for invalid `MARCUS_ASK_B_QUERY_MAX_CHARS` (R7); (6) canonical query is now a delimiter-neutralized, whitespace-collapsed derived-display projection (R5b/c) — every one of these changes downstream digests, which is exactly why they land pre-witness. Non-contract hardening: init-order fix (R1a), exhaustive per-row losses (R1b), UnicodeDecodeError routing (R1d), full-shape stub gates at BOTH band coordinates (R3, symmetric hardening), forged-association + duplicate-contribution strict-reader rejections (R4), shared `run.json` symlink guard (R9), machine-forced pending-witness flip + `owed_by_probe` registry field (R10), intake replay-divergence coverage (R12), platform-guarded directory fsync (R13). Row-14 traceability corrected to name `test_ask_conflicting_completed_contribution_fails_split_brain` (R2). Fences verified untouched: `production_runner.py` / `pipeline-manifest.yaml` / `trends_projection.py`.
