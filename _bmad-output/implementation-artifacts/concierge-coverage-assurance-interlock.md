...
**AC-OP2 — HIGH-CAPABILITY MODEL (essential to scholarly content delivery).** Every LLM-backed function — (a) presentation-note → teaching-assertion segmentation, (b) coverage-intent refinement at ambiguity, (c) content-presence judgment at a resolved anchor, (d) any audit/report-generation LLM assist — MUST run on a **gpt-5-class frontier model** (the intended production model), **real calls, NO mocks**. **Determinism (gpt-5 gotcha):** gpt-5 REJECTS `temperature=0` (400). Achieve determinism via **pinned model + `seed` + fixed prompt template + prompt-hash logged + first-run-stands / no-retry-to-green**, temperature bound at CONSTRUCTION (mirror `_live_pre_pass` in the P3 pass; do NOT pass per-call temperature). **Reuse the existing gpt-5 live-model binding path that `build_pedagogy_annotations` already uses** — do not introduce a new model seam.
...
- [ ] **T0/T2 INGEST-SEGMENTATION SPIKE — FIRST, gating** (AC: OP1, OP2, 2) — prove assertion-level re-segmentation of REAL presentation notes (the markdown `**Narration (Speaker Notes):**` convention; substrate: `course-content/courses/tejal-apc-c1-m1-p2-trends/slides/*.md`) is **bounded** via the gpt-5 segmentation pass. Write a NUMERIC escalation threshold in the AC (e.g. max assertions/block, max latency/pass, max retries=0). Outcome: bounded → segmenter ships active (assertion_level); unbounded/unreliable → **HALT + ESCALATE to operator** (no coarse fallback). Capture spike evidence.
