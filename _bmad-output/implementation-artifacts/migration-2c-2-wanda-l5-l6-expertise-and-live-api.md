...
**Epic:** Slab 2c (migration Epic 2c â€” Wondercraft Pilot + Generator Validation) â€” **second story; Path-A-residual completion**.
**Pts:** 3 | **Gate:** single (per governance JSON `2c-2.expected_gate_mode = "single-gate"`, rationale: null â€” expertise-population + deferred-live-API completion; no schema-shape, no lane-boundary, no invariant-preservation). **K-target:** ~1.3Ã— (target 10 / floor 8; documentation-heavy + sanctum-population + ONE new live-API integration test + L5/L6 fingerprint regression).
...
- **A-R4-2c.2:** AC-D-OP-FALLBACK added â€” if `create_scripted_podcast` exceeds $5 ceiling, operator may run `create_podcast` (shorter format) at $1-2 ceiling; live artifact marked "scripted preferred, simple acceptable."
- **A-R5-2c.2:** AC-A reframed â€” dev creates directory + placeholder skeleton files with `<!-- TODO: operator-populate via First Breath -->` markers + minimal valid frontmatter; operator populates persona content in own session before AC-G. AC-A acceptance is **structural** (files exist, git-tracked, valid frontmatter), NOT content-quality.
- **A-R6-2c.2:** AC-A clarifies "INDEX.md is operator-facing reference; runtime does not parse it" â€” prevents over-engineering as runtime manifest.
- **M-R18 (recall) â€” K-floor honesty:** AC-B `test_wanda_references_tuple_contains_l5_paths` 3-case parametrize collapses to 1 K-floor unit (membership-echo, not orthogonal property). Honest K-floor: 7. AC-B gains ONE behavior test `test_read_wanda_references_includes_l5_bodies_in_declared_order` to legitimately reach K-floor 8 (asserts `_read_wanda_references()` output contains each L5 file body in declared tuple order; verifies read path resolves the new sub-directory + ordering preserved).
...
- **M-R32-2c.2 (NEW) â€” RETIRED-MOOT:** addopts merge concern obviated by `--run-live` opt-in mechanism (no `-m` filter in `addopts` to merge with).
- **M-R33-2c.2 (NEW):** AC-D live-artifact duration assertions MUST be range-bounded (lower AND upper): `4*60 â‰¤ duration_sec â‰¤ 10*60`. Upper bound is runaway-generation canary paired with Decision #4 cost ceiling.
...
- **Live API artifact (per 2b.8 AC-B-OP):** "DEFERRED-PENDING-OPERATOR-WINDOW" â€” no live podcast artifact has been produced.
...
1. **Governance lookup** â€” `docs/dev-guide/migration-story-governance.json` confirms `2c-2.expected_gate_mode = "single-gate"` (rationale: null). Do not relitigate.
2. **2b.8 close evidence** â€” `_bmad-output/implementation-artifacts/migration-2b-8-wanda-wondercraft-scaffold-migration.md` AC-B-OP DEFERRED block + AC-D graceful-degrade decision. Story 2c.2 EXECUTES the deferred items; does NOT re-author Wanda's `graph.py`/`wondercraft_dispatch.py`.
3. **Wanda runtime** â€” `app/specialists/wanda/{graph.py, wondercraft_dispatch.py, state.py, model_config.yaml}` shipped. **DO NOT TOUCH `_act` orchestration**; only extend `_read_wanda_references` tuple + `WANDA_REFERENCES_DIR` constant if L5 reference files land in a new sub-directory (Decision #2 below).
4. **Wondercraft client substrate** â€” `scripts/api_clients/wondercraft_client.py` exposes `check_connectivity / list_episodes / create_podcast / create_scripted_podcast / create_conversation_podcast / get_job_status / wait_for_job`. Reads `WONDERCRAFT_API_KEY` from `.env` at instantiation.
5. **TEMPLATE doc** â€” [`docs/dev-guide/specialist-migration-template.md`](../../docs/dev-guide/specialist-migration-template.md) v2.4 R1â€“R14. Most rules N/A (no migration in scope; this is post-migration enrichment). R7 resolution-trail uniformity preserved (no `_plan` semantics changed). R8 K-floor rules apply.
6. **Live API marker mechanism (verified 2026-04-26 per A-R1 BLOCKER resolution)** â€” `live_api` marker is ALREADY registered at `pyproject.toml:62-63` AND `tests/conftest.py:88` (and `live_api_e2e` companion). Skip mechanism: `--run-live` opt-in via `pytest_collection_modifyitems` at `tests/conftest.py:129-149` (Pass 1 deselects `live_api` items unless `--run-live` flag passed). NO `addopts -m "not live_api"` filter exists; do NOT add one. Default `pytest` invocation deselects live_api tests automatically; explicit `--run-live` opts them in.
7. **Sanctum BMB pattern** â€” `_bmad/memory/bmad-agent-marcus/{INDEX.md,PERSONA.md,chronology.md,access-boundaries.md}` is the canonical full-population reference. Wanda's sidecar directory follows the **sidecar** convention (`_bmad/memory/wanda-sidecar/`) per Wanda's existing `SANCTUM_DIR` constant at `app/specialists/wanda/graph.py:26`, NOT the `bmad-agent-wanda/` BMB layout (Wanda is a non-BMB specialist; sidecar pattern is correct per Slab-2b precedent â€” `kira-sidecar`, `vyx-sidecar`, etc.).
8. **Severance posture** â€” hybrid working tree is sole input surface.
9. **Predecessor close evidence** â€” Story 2c.1 expected `done` before 2c.2 opens (M2 evidence dependency: 2c.1 produces the diff-evidence Markdown; 2c.2 does NOT depend on it but assumes Wanda's runtime is unchanged from 2b.8 close + 2b.15 dispatch family completion).
...
### AC-2c.2-A â€” `_bmad/memory/wanda-sidecar/` skeleton creation (dev structural; operator content-population per A-R5)
...
- **When** the dev agent creates the directory tree with **skeleton placeholder files** carrying minimal valid frontmatter + `<!-- TODO: operator-populate via First Breath ceremony -->` markers:
  ...
  â”œâ”€â”€ INDEX.md                        # operator-facing reference index (NOT runtime-parsed per A-R6); references all sanctum files + L6 path
  â”œâ”€â”€ PERSONA.md                      # skeleton: title + 1-line role + TODO marker; operator populates persona/voice/capability-scope
  â”œâ”€â”€ access-boundaries.md            # skeleton: title + Wondercraft cost-ceiling note from Decision #4 + TODO marker for operator verbs
  â”œâ”€â”€ chronology.md                   # one entry: "2c.2 sanctum skeleton created 2026-04-26 by dev agent; operator-population pending"
  â””â”€â”€ L6-operational/
      â””â”€â”€ wondercraft-context.md      # skeleton: required-section headers (## Voice-ID catalog / ## Episode-template skeleton / ## Style-guide overrides) + TODO markers per section; operator populates content per Decision #3
  ```
- **Then** `_read_sanctum_digest()` returns a non-empty newline-separated list of `<rel-path>\t<sha256>` lines for the 5 sanctum files (sorted lexically); cache-prefix CRLFâ†’LF normalization preserved per Wanda 2b.8 implementation at `app/specialists/wanda/graph.py:80-96`.
- **Authoring authority pin per A-R5:** dev creates **structural skeletons + TODO markers + valid frontmatter ONLY** â€” content quality (full persona narrative, voice-ID catalog values, access-boundary verb list) is **operator territory** populated via First Breath ceremony in operator's own session before AC-G party-mode review. AC-A acceptance is **structural** (files exist + are git-tracked + parse as valid Markdown + carry TODO markers + AC-A regex-pinned section headers present), NOT content-quality.
...
- **Test pin:** `tests/specialists/wanda/test_wanda_sanctum_populated.py` â€” 3 tests (parametrize-collapsible per Murat M-R18 â†’ 1 K-floor unit):
  1. `test_sanctum_directory_exists` â€” `_bmad/memory/wanda-sidecar/` present + contains all 5 expected skeleton files.
  2. `test_sanctum_digest_nonempty_post_population` â€” `_read_sanctum_digest()` returns non-empty string + 5 lines + each line is `<path>\t<64-hex-sha256>`.
  3. `test_sanctum_digest_deterministic_under_crlf` â€” write a sanctum file with CRLF newlines, recompute digest, assert equals LF-equivalent (pins CRLFâ†’LF normalization against future regression).

### AC-2c.2-B â€” L5 podcast-production reference files at `skills/bmad-agent-wondercraft/references/L5-podcast-production/` + Wanda wiring
...
- **When** the dev agent authors 3 NEW reference files in `skills/bmad-agent-wondercraft/references/L5-podcast-production/`:
  - `storytelling-framework.md` â€” narrative arc patterns for educational podcast (hook â†’ context â†’ conflict â†’ resolution â†’ call-to-action); â‰¥40 lines substantive content
  - `audio-production-patterns.md` â€” voice-acting cadence + pause-discipline + music-bed-volume-curve patterns; â‰¥40 lines substantive content
  - `narration-length-vs-engagement.md` â€” minute-bracket guidance for retention (3-min "explainer" / 8-min "deep-dive" / 15-min "story-arc") with citation pointers; â‰¥40 lines substantive content
...
- **Test pins:**
  1. `tests/specialists/wanda/test_wanda_l5_references_present.py` â€” 3 tests:
     - `test_l5_directory_and_three_files_exist` â€” `skills/bmad-agent-wondercraft/references/L5-podcast-production/` present + all 3 files exist + each â‰¥40 lines.
     - `test_wanda_references_tuple_contains_l5_paths` â€” `WANDA_REFERENCES` is a 14-tuple (11 original + 3 new); the 3 new entries match exactly the L5 file paths (membership-echo per Murat M-R18; 1 K-floor unit).
  ...
  2. **Existing test extension** at `tests/specialists/wanda/test_wanda_plan_node.py` (or equivalent) â€” assert `_read_wanda_references()` output length grew from 11-segment to 14-segment baseline; no regression on the original 11 segments.

### AC-2c.2-C â€” L6 operational context at `_bmad/memory/wanda-sidecar/L6-operational/wondercraft-context.md`
...
- **When** the dev agent authors `wondercraft-context.md` containing:
  - **Voice-ID catalog:** â‰¥3 production voices with role mapping (e.g., `voice_a_narrator: "Vivian"`, `voice_b_expert: "Marcus"`, `voice_c_student: "Lena"`); each entry annotated with usage-context + register + sample-rate cap.
  ...
  - **Style-guide overrides:** music-bed-volume default âˆ’18LUFS; pause-after-question 1.2s; segue-tone preference per voice-pair.
  - File â‰¥60 lines.
...
- **Test pin:** `tests/specialists/wanda/test_wanda_l6_operational_context.py` â€” 1 test: `test_l6_wondercraft_context_present_and_substantive` â€” file exists + â‰¥60 lines + parses for the three required sections (regex `^## Voice-ID catalog`, `^## Episode-template skeleton`, `^## Style-guide overrides`).

### AC-2c.2-D â€” Live Wondercraft API integration test (`@pytest.mark.live_api`; deselected by default per `--run-live` opt-in mechanism)
...
- **And** `WONDERCRAFT_API_KEY` may or may not be available in the dev-agent's `.env` (sandbox-AC discipline: dev-agent ACs verify via shipped Python dep + `pytest.skip(...)` when absent â€” composed with the `--run-live` opt-in)
- **When** the dev agent authors `tests/specialists/wanda/test_wanda_live_api_artifact.py`:
  ...
  @pytest.mark.skipif(
      ...
      reason="WONDERCRAFT_API_KEY absent â€” live API test deferred to operator window",
  )
...
- **Range-bounded duration assertion per Murat M-R33 (binding):** `4*60 <= duration_sec <= 10*60` (lower bound: artifact is substantive enough to validate quality; upper bound: runaway-generation canary paired with Decision #4 cost ceiling â€” duration drift correlates with cost drift on Wondercraft per-second pricing).
- **Test pins:**
  1. `tests/specialists/wanda/test_wanda_live_api_artifact.py` â€” 1 live test (deselected by default; opt-in via `--run-live`; counts as 1 K-floor unit).
  2. `tests/specialists/wanda/test_wanda_live_api_marker_skipped_in_default_run.py` â€” 1 NEGATIVE test asserting that **default `pytest` invocation** (no `--run-live`) deselects the live_api test, AND **`--run-live` invocation** selects it. Verified via:
     ```python
     import os
     import subprocess
     import sys
...
### AC-2c.2-D-OP â€” Operator-gated live execution + artifact paste (DEFERRED-PENDING-OPERATOR-WINDOW allowed; Path-A residual closes 2b.8 AC-B-OP)
...
- **Then** operator runs (per A-R1-RESOLVED `--run-live` mechanism, NOT `-m live_api`):
  ...
  ```
- **And** pastes into Dev Agent Record Â§"Operator Gate â€” AC-2c.2-D-OP":
...
- **And** the artifact + metadata files committed to git (these are evidence artifacts and small enough â€” verify â‰¤10 MB MP3 per Wondercraft 8-min default).
- **Cost ceiling per Decision #4:** $5.00 single-artifact ceiling for `create_scripted_podcast`. If the live job exceeds this, abort the test + file `2c-2-live-api-cost-overrun-rca` follow-on. **NOT a 2c.2 close-blocker if deferred** â€” DEFERRED-PENDING-OPERATOR-WINDOW marker preserved through close; reactivation gate "operator ratifies cost + opens window."
...
- **Cost-vs-2c.1 reconciliation:** 2c.1 AC-B-OP smoke-test ($0.50) and 2c.2 AC-D-OP production-quality ($5.00 scripted OR $1-2 simple-fallback) are **distinct** operator windows with **distinct artifacts**. They MAY be combined into a single operator session (executed back-to-back) but the artifacts MUST be separately stored (2c.1 â†’ `tests/fixtures/specialists/wanda_validation/...`; 2c.2 â†’ `tests/fixtures/specialists/wanda/live_artifacts/...`).

### AC-2c.2-E â€” Filesystem sha256 â†” LIVE_ARTIFACT_METADATA.md round-trip (NARROWED per A-R2 BLOCKER resolution)
...
- **Receipt-key extraction OUT OF SCOPE for 2c.2** per A-R2 narrowing â€” `parsed.path/duration_sec/voice_id/sha256` strict-typing on the receipt is **deferred** to the receipt-strict-typing follow-on (`2c-2-receipt-strict-typed-artifact-metadata`) which would require coordinated edits across receipt model + dispatch helpers + 2b.8 substrate (out of Decision #1 bounded scope).
- **Test pin:** `tests/specialists/wanda/test_wanda_live_artifact_metadata_round_trip.py` â€” 1 test (skipped if AC-D-OP defers; not a close-blocker; auto-files deferred-inventory entry at close).

### AC-2c.2-F â€” Anti-pattern catalog harvest (per R6)

- **Given** the catalog at `docs/dev-guide/specialist-anti-patterns.md` is final-form post-2b.17 + 2b.15 with A1â€“A13 entries
...
### AC-2c.2-G â€” TEMPLATE compliance (per R1â€“R14 v2.4)

R1â€“R14 v2.4 honored where applicable. **Most rules N/A** (no migration; this is post-migration enrichment). Applicable: R1 bounded scope (Decision #1); R6 framework-drift harvest (one divergence captured at Â§Epic-doc-vs-runtime cross-check (a)); R7 resolution-trail uniformity preserved (no `_plan` signature change); R8 K-floor recalibration (~1.3Ã— = target 10 / floor 8 honored).

### AC-2c.2-H â€” D12 close protocol (single-gate; FOUR-line per A-R7)
...
3. **Migration-guide update:** Â§12.11 (Gary REST-API category) gains a one-line note "Wanda L5/L6 enrichment + live-API exercise at 2c.2; sanctum populated to `_bmad/memory/wanda-sidecar/`."
...
### AC-2c.2-I â€” Sprint-status state-flips at filing AND at close
...
â”œâ”€â”€ INDEX.md
â”œâ”€â”€ PERSONA.md
â”œâ”€â”€ access-boundaries.md
â”œâ”€â”€ chronology.md
â””â”€â”€ L6-operational/
    â””â”€â”€ wondercraft-context.md

skills/bmad-agent-wondercraft/references/L5-podcast-production/
â”œâ”€â”€ storytelling-framework.md
â”œâ”€â”€ audio-production-patterns.md
â””â”€â”€ narration-length-vs-engagement.md

tests/specialists/wanda/
â”œâ”€â”€ test_wanda_sanctum_populated.py                              # 3 tests (AC-A; parametrize-collapse â†’ 1 K-floor unit)
â”œâ”€â”€ test_wanda_l5_references_present.py                          # 3 tests (AC-B; includes M-R18 honest-add behavior test)
â”œâ”€â”€ test_wanda_l6_operational_context.py                         # 1 test (AC-C)
â”œâ”€â”€ test_wanda_live_api_artifact.py                              # 1 test (AC-D; deselected by default per --run-live opt-in)
â”œâ”€â”€ test_wanda_live_api_marker_skipped_in_default_run.py         # 1 NEGATIVE test (AC-D; PYTEST_ADDOPTS-sanitized subprocess per M-R31)
â””â”€â”€ test_wanda_live_artifact_metadata_round_trip.py              # 1 test (AC-E; filesystem sha256 round-trip; skipped if DEFERRED; auto-files follow-on)
```

### MODIFIED files

- `app/specialists/wanda/graph.py` â€” `WANDA_REFERENCES` tuple extended from 11 to 14 entries (additive, AC-B). NO other changes (Decision #1 + A-R3 reframe permits this minimal extension).
- `docs/dev-guide/specialist-migration-template.md` â€” section Â§12.11 gains the AC-H one-line note.
- `_bmad-output/implementation-artifacts/sprint-status.yaml` â€” per AC-I.
- `_bmad-output/planning-artifacts/deferred-inventory.md` â€” `2c-2-mcp-architecture-divergence-from-epic-literal` follow-on filed per Â§Epic-doc-vs-runtime cross-check (a). `2c-2-ac-d-op-live-wondercraft-evidence` filed if AC-D-OP defers. `2c-2-ac-e-round-trip-pending-live-artifact` auto-filed if AC-D-OP defers (per M-R34). `2c-2-receipt-strict-typed-artifact-metadata` filed at story open per A-R2 narrowing.

### NOT modified (per A-R1 RESOLVED + A-R2 RESOLVED-WITH-NARROWING)

- `pyproject.toml` `[tool.pytest.ini_options].markers` â€” `live_api` ALREADY registered at lines 62-63 (verified 2026-04-26). NO modification.
- `tests/conftest.py` â€” `--run-live` flag + `pytest_collection_modifyitems` deselect logic ALREADY in place at lines 68-149. NO modification.
- `app/models/dispatch/wanda/receipt.py` â€” loose-typed `wanda_audio` field stays (strict-typing of artifact metadata keys deferred per A-R2 narrowing).
- `app/specialists/wanda/wondercraft_dispatch.py` â€” DO NOT TOUCH (2b.8 architectural pin).

### CONDITIONALLY MODIFIED files (operator-paste landing zone)

- `tests/fixtures/specialists/wanda/live_artifacts/2026-04-26/<sha256>.mp3` â€” operator-pasted live artifact (AC-D-OP) â€” committed if AC-D-OP fires.
- `tests/fixtures/specialists/wanda/live_artifacts/2026-04-26/LIVE_ARTIFACT_METADATA.md` â€” operator-pasted metadata (AC-D-OP) â€” committed if AC-D-OP fires.

### NOT modified (additional)

- `scripts/api_clients/wondercraft_client.py` â€” DO NOT TOUCH (substrate stable).
- `app/models/dispatch/wanda/{input,error}.py` â€” DO NOT TOUCH (2b.15 strict-typed family preserved).
- `app/specialists/wanda_validation/` â€” DO NOT TOUCH (Path B 2c.1 transient validation tree, retired/discarded at 2c.1 close).

---

## Testing Requirements

**K-target ~1.3Ã— (target 10 / floor 8).** Test count and K-floor accounting per Murat M-R18 honest-count discipline:
...
**Honest K-floor: 8** (with AC-E firing when AC-D-OP completes), **7 firm** (AC-E skipped if deferred). Conforms to ~1.3Ã— K-target (target 10 / floor 8 â‰¤ 1.25Ã— when AC-E fires; â‰¤ 1.43Ã— when AC-E defers â€” both within the K-target band). The M-R18 honest-add `test_read_wanda_references_includes_l5_bodies_in_declared_order` legitimately raises K-floor to 8 by adding an orthogonal property (read-path + ordering behavior), not a membership-echo inflation.

**Regression target at T8:** â‰¥562 passed / â‰¥7 skipped placeholder-key (Slab 2b close baseline preserved); +10 collected at file level; deselect-by-default `live_api` mechanism keeps CI green; AC-E may add 1 skip if AC-D-OP defers. Import-linter 3/3 KEPT throughout. Ruff clean. Sandbox-AC PASS (live API verified via shipped Python dep `WondercraftClient`, gated by `pytest.skip(...)` on missing env var + `--run-live` opt-in).
...
- 2026-04-26: Loaded BMAD config, `CLAUDE.md`, project context, full 2c.2 spec, sprint status, governance JSON, TEMPLATE v2.4, Wanda runtime, Wondercraft client substrate, pytest live marker mechanism, Wanda dispatch receipt shape, and Wondercraft skill references.
- Predecessor gate passed: Story 2c.1 is `done` in sprint status and committed at `6ddf338`; persistent M2 diff anchor exists at `_bmad-output/implementation-artifacts/2c-1-wondercraft-path-b-diff-evidence.md`.
- Governance confirmed: `docs/dev-guide/migration-story-governance.json` has `2c-2.expected_gate_mode = "single-gate"` with null rationale; gate mode not relitigated.
- Artifact sweep passed: Wanda runtime files exist; `_bmad/memory/wanda-sidecar/` absent before AC-A; Wondercraft reference substrate exists with 11 parent files and no L5 podcast-production subdirectory; `live_api` marker and `--run-live` deselection mechanism are already present; `WandaDispatchReceipt.wanda_audio` is loose-typed.
- Sandbox-AC validator passed: `.venv\Scripts\python.exe scripts/utilities/validate_migration_story_sandbox_acs.py _bmad-output/implementation-artifacts/migration-2c-2-wanda-l5-l6-expertise-and-live-api.md`.
- T1 disposition: PASS; no halt condition.

### T2-T7 Implementation Notes

- AC-A: Created `_bmad/memory/wanda-sidecar/` structural skeleton with five files, valid frontmatter, TODO markers, and required operator-facing sections; `_read_sanctum_digest()` now returns five sorted sha256 lines.
- AC-B: Authored three L5 podcast-production references under `skills/bmad-agent-wondercraft/references/L5-podcast-production/` and extended `WANDA_REFERENCES` from 11 to 14 entries without changing Wanda dispatch or `_act` behavior.
- AC-C: Authored L6 operational context skeleton at `_bmad/memory/wanda-sidecar/L6-operational/wondercraft-context.md` with required headers and operator TODO markers; content remains operator-population pending per A-R5.
- AC-D: Added `@pytest.mark.live_api` Wondercraft artifact test plus default-deselection negative tests using sanitized `PYTEST_ADDOPTS`; live execution not run by dev agent.
- AC-E: Added filesystem sha256 metadata round-trip test that skips with explicit AC-D-OP deferral reason until `LIVE_ARTIFACT_METADATA.md` and MP3 artifact land.
- AC-F/AC-G/AC-H: No new anti-pattern harvested; migration-guide Section 12.11 note added; TEMPLATE R1/R6/R7/R8 constraints preserved.

### T8 Regression Evidence

- PASS: `.venv\Scripts\python.exe -m pytest tests/specialists/wanda -q` -> 38 passed, 1 skipped, 1 deselected.
- PASS: `.venv\Scripts\python.exe -m pytest tests/integration/scaffold_conformance/ -q` -> 58 passed.
- PASS: scoped ruff over `app/specialists/wanda/graph.py` and six new Wanda test files.
- PASS: `.venv\Scripts\lint-imports.exe --config pyproject.toml` -> 3/3 contracts KEPT.
- PASS: sandbox-AC validator for 2c.2.
...
- OPERATOR-ACCEPTED BASELINE DRIFT: `.venv\Scripts\python.exe -m ruff check app/ tests/ skills/ scripts/` still reports the broad pre-existing lint backlog outside the 2c.2 diff.
...
- Live artifact disposition: AC-D-OP deferred; AC-E skip remains in tree and deferred-inventory entries filed.

### Operator Gate - AC-2c.2-D-OP Live Wondercraft Evidence

DEFERRED-PENDING-OPERATOR-WINDOW. Dev agent did not run live Wondercraft API or incur cost. Deferred-inventory entries `2c-2-ac-d-op-live-wondercraft-evidence` and `2c-2-ac-e-round-trip-pending-live-artifact` filed.

### G6 Layered Code-Review (Blind Hunter / Edge Case Hunter / Acceptance Auditor)

- Blind Hunter: Wanda runtime behavior is unchanged except the additive reference tuple; dispatch wrapper and receipt model remain untouched per scope.
- Edge Case Hunter: live API test is deselected by default and the negative subprocess test clears `PYTEST_ADDOPTS`; metadata round-trip test skips explicitly when artifact is absent.
- Acceptance Auditor: AC-A through AC-H implementation evidence exists; AC-D-OP is correctly deferred and represented in tests plus deferred inventory; no 2c.2 MUST-FIX findings remain.
- `bmad-code-review` scoped result: no MUST-FIX findings in the 2c.2 diff. Residual risks are deferred live artifact execution, loose receipt metadata typing deferred by design, and repo-wide baseline failures already documented.

### D12 Close Stub

1. **Invariant preservation:** Wanda `_act`, dispatch wrapper, and dispatch models remain untouched; only data substrate, reference tuple, docs, and tests changed.
2. **Anti-pattern harvest:** N/A; MCP epic-literal divergence and receipt strict-typing are filed as deferred follow-ons rather than catalog entries.
3. **Migration-guide update:** Section 12.11 records Wanda L5/L6 enrichment and deferred live-API exercise without changing the REST-API dispatch category.
4. **TEMPLATE compliance:** R1, R6, R7, and R8 honored; AC-D-OP live artifact deferred under operator-gated cost discipline.

### Completion Notes

- BMAD-CLOSED 2026-04-26T01:26:46-04:00. Wanda L5/L6 expertise substrate and live-API validation scaffolding are in place.
- `WANDA_REFERENCES` now includes 14 files, with the three L5 podcast-production files loaded in declared order.
- AC-D-OP live Wondercraft artifact is deferred; metadata round-trip validation will activate when the operator-run artifact lands.
- Deferred-inventory count is now 33 named follow-ons after four 2c.2 entries.

### File List

- `_bmad/memory/wanda-sidecar/INDEX.md`
- `_bmad/memory/wanda-sidecar/PERSONA.md`
- `_bmad/memory/wanda-sidecar/access-boundaries.md`
- `_bmad/memory/wanda-sidecar/chronology.md`
- `_bmad/memory/wanda-sidecar/L6-operational/wondercraft-context.md`
- `_bmad-output/implementation-artifacts/migration-2c-2-wanda-l5-l6-expertise-and-live-api.md`
...
- `skills/bmad-agent-wondercraft/references/L5-podcast-production/storytelling-framework.md`
- `skills/bmad-agent-wondercraft/references/L5-podcast-production/audio-production-patterns.md`
- `skills/bmad-agent-wondercraft/references/L5-podcast-production/narration-length-vs-engagement.md`
- `tests/specialists/wanda/test_wanda_sanctum_populated.py`
- `tests/specialists/wanda/test_wanda_l5_references_present.py`
- `tests/specialists/wanda/test_wanda_l6_operational_context.py`
- `tests/specialists/wanda/test_wanda_live_api_artifact.py`
- `tests/specialists/wanda/test_wanda_live_api_marker_skipped_in_default_run.py`
- `tests/specialists/wanda/test_wanda_live_artifact_metadata_round_trip.py`