...
## Deferred from: Story Q3.1 — capability-honesty dimension (2026-07-19)

- source_spec: `q3-1-capability-honesty-dimension.md` (AC2/AC6, PHASING FLAG — Q2-retro AI-Q2d) — **SPLIT TODO: full trial-artifact-scan auto-reconciliation.**
  summary: `q3-1-full-trial-artifact-scan-reconciliation` — Q3.1 ships the capability_honesty SCORING + a **BOUNDED** reconciliation: `app/quality/signals.py::capability_tier_reconciliation_signal` reads the REAL `bundle_catalog` `CAPABILITY_TIERS` (read-only) against a SMALL, CURATED produced-evidence set (`_CURATED_PRODUCED_EVIDENCE` — the recorded DID-Leak-5 evidence: workbook produced MD+DOCX @ trial `a940c5eb`, LO-verified `8b275e5b`, frozen Tejal P2). The DEEP reconciliation — "scan EVERY tracked trial run's artifacts (workbook.md/.docx, motion, deck) to derive produced-reality per component, instead of a hand-curated evidence set" — is DEFERRED here per the binding PHASING FLAG (ship scoring + a bounded reconciliation; split the deep auto-scan if it over-balloons). When built: (1) enumerate produced artifacts per component across `state/config/runs/<trial_id>/` evidence; (2) feed that derived produced-evidence into `capability_tier_reconciliation_signal(produced_evidence=<scanned>)` (the reader already takes an injectable `produced_evidence` — the seam exists); (3) it widens the reconciliation toward every produced component and may surface the motion VERIFY below as a real leak. **Do NOT edit a `bundle_catalog` tier from this work** — a tier bump is party-gated governance; this only SCORES honesty. **Reactivation trigger: when ≥1 tracked trial's artifact evidence is stable enough to scan, or at the 39/40-wave close.**
  evidence: bounded reconciliation + dimension + pins landed (`app/quality/signals.py`, `docs/quality/project-quality-scorecard.md` §5, `tests/quality/test_capability_honesty_dimension.py`, `tests/quality/test_scorecard_honesty_pins.py`); `bundle_catalog.py` UNCHANGED — consumed read-only (no tier edited; no parallel plumbing — GL-15). Epic source: `epics-project-quality-scorecard-2026-07-19.md` §Story Q3.1 (FR10) + the PHASING FLAG.

- source_spec: `q3-1-capability-honesty-dimension.md` (Honest baseline VERIFY — NOT a counted leak) — **MOTION CAPABILITY-TIER VERIFY.**
  summary: `q3-1-motion-capability-tier-verify` — the honest baseline flags that motion's `bundle_catalog` tier `proven_regressed_repairable` is **likely stale the same way the workbook tier lagged** — re-read it against LIVE motion output. This is a **VERIFY TODO, NOT a counted capability leak** (it is not in the `## Capability-Honesty Scorecard Leak Registry` and does not touch `open_leaks`). If a re-read shows motion has produced a real artifact recently (or genuinely still fails-loud), reconcile the tier posture — but ANY tier change is **party-gated governance** (do NOT edit it from the scorecard). If the VERIFY confirms a lag/overstatement, add curated produced-evidence for motion to `_CURATED_PRODUCED_EVIDENCE` so the reconciliation flags it (turning the VERIFY into a counted leak only then). **Reactivation trigger: next motion-track touch, or when the split trial-artifact scan (above) lands.**
  evidence: recorded in the §5.6 assessment + the CH1 signal `caveat` + the DID-Leak-5 deferred entry (`workbook-capability-tier-honesty-lag`) `VERIFY` note. Epic source: `epics-project-quality-scorecard-2026-07-19.md` §Story Q3.1 (Honest baseline).

- source_spec: `q3-1-capability-honesty-dimension.md` (AC6 / GL-10) — **R2 CAPABILITY-HONESTY WITNESS OBLIGATION (checkable comparison, not an observation).**
  summary: `q3-1-r2-capability-honesty-witness` — Q3.1 ships the capability_honesty dimension + its reconciliation reader (`app/quality/signals.py::{capability_tier_reconciliation_signal, capability_leak_count_signal}`) + honesty pins OFFLINE, scored by reconciling the EXISTING `bundle_catalog` tiers READ-ONLY against curated produced-evidence (GL-15). The LIVE end-to-end witness is DEFERRED to the Operator/HIL **R2 trial** (per GL-10; `bundle_catalog.py` is consumed read-only and `production_runner` is a pipeline-lockstep trigger path, deliberately untouched here). On R2: (1) capture which bundle the run OFFERED + its `bundle_readiness` verdict; (2) assert `capability_tier_reconciliation_signal()` still reports the real `CAPABILITY_TIERS` posture — the workbook lag (`tiers_match_produced_reality=False` → CH1 `weak`) — independent of the run, and that no tier was edited by the run; (3) if a run produced a workbook artifact and the tier was PARTY-RATIFIED to match, assert the reader reports `tiers_match_produced_reality=True` and CH1 earns `strong` (the reachable close-path); (4) a mismatch = a real capability-emit/reader defect. Do NOT run a live trial to satisfy this story; it rides the already-planned R2 shakedown.
  evidence: reader + dimension + pins landed (`app/quality/signals.py`, `docs/quality/project-quality-scorecard.md` §5, `tests/quality/test_capability_honesty_dimension.py`, `tests/quality/test_scorecard_honesty_pins.py`); `bundle_catalog.py` UNCHANGED (consumed read-only, NO tier edited — the governance fence; no parallel plumbing — GL-15). Epic source: `epics-project-quality-scorecard-2026-07-19.md` §Story Q3.1 (FR10) + GL-10.
