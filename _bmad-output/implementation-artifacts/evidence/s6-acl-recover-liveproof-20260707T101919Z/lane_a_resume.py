"""S6 AC-L RECOVER lane (a) RESUME — finish the paused-at-G1 trial 181c6621.

The lane-a walk cleanly reached paused-at-gate G1 (G0E->G0R->G1 proven; D1 flip +
creds proven) but the process was terminated mid-G1-resume. irene_pass1 is cached
in the persisted envelope, so this RESUMES trial 181c6621 from G1 (approve) -> the
continuation walk crosses node 04.55 (the D7-reachable Scite dispatch) -> next
pause. Same facts-capture + D7 provenance as lane_a_driver. NO restart, NO re-freeze.
"""
...
TRIAL_ID = UUID("181c6621-d706-4f1f-807c-f51a7998657d")
HARD_TIMEOUT_S = 560
...
assert not env_dispatch_present, "MARCUS_RESEARCH_DISPATCH_LIVE must be ABSENT by name"
...
log(f"RESUME env: dispatch_live_fn={dispatch_fn} creds={creds} ready={ready}")
assert dispatch_fn is True and creds is True
...
try:
    run = read_run()
    log(f"RESUME start: on-disk status={run.get('status')} paused_gate={run.get('paused_gate')}")
    assert run.get("paused_gate") == "G1", f"expected paused at G1, found {run.get('paused_gate')}"
    ...
    contrib = prod.get_contribution(RESEARCH_WIRING_SPECIALIST_ID, node_id=RESEARCH_WIRING_NODE_ID) if prod else None
    if contrib is None:
        log("NO research_wiring contribution — walk did not reach 04.55")
