"""S5-3b AC-L lane (a) — RESUME G0E (approve) via the real runner resume path.

Reads the persisted decision-card-G0E.json, builds an approve OperatorVerdict, and
resumes; the next pause must be G0R (LO ratify).
"""
...
os.environ.pop("MARCUS_G0_ENRICHMENT_ACTIVE", None)  # lane (a): default unset
...
try:
    ...
    log(f"{GATE} verdict: approve (confirm enrichment); resuming with cap=8")
