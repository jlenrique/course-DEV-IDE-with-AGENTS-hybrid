"""S5-3b AC-L lane (b) — RESUME G0E (approve) via the real runner resume path.

The G0-enrichment node is already frozen (idempotent) from the armed start, so this
resume does NOT re-invoke the live LLM; it advances past the G0E confirm gate to G0R.
The armed env is kept set for fidelity.
"""
...
os.environ["MARCUS_G0_DISPATCH_LIVE"] = "1"  # armed-lane fidelity (node already frozen; no re-call)
...
try:
    ...
    log(f"{GATE} verdict: approve; resuming with cap=8")
    envelope = resume_production_trial(
        trial_id=TRIAL_ID, verdict=verdict, runs_root=RUNS_ROOT, max_specialist_calls=8
    )
...
(EVIDENCE / f"lane_{LANE}-resume-g0e-result.json").write_text(
