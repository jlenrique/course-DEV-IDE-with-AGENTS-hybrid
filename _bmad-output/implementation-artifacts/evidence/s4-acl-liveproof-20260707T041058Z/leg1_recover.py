"""S4 AC-L LEG-1 driver, phase 2: recover the rewound copy at gary@07.

Gary re-enters at §07 and reads gamma_settings from the edited directive.yaml:
variant A present with NO styleguide key. Post-S4 Flip A must HALT PRE-SPEND:
error-pause at gamma.styleguide.unbound, ZERO Gamma generative spend, message
...
result: dict = {"leg": 1, "phase": "recover", "trial2": str(TRIAL2)}

t0 = datetime.now(tz=UTC)
try:
    log(f"recover_production_trial({TRIAL2}) with cap=40 — gary re-enters §07 with a "
        "named styleguide-less variant (S4: expect HALT at gamma.styleguide.unbound)")
...
ok = (
    ...
    and result.get("paused_error_tag") == "gamma.styleguide.unbound"
)
log(f"=== LEG1 PHASE2 {'OK (HALTED as expected)' if ok else 'NOT-OK'} after {result['elapsed_s']:.0f}s ===")
timer.cancel()
