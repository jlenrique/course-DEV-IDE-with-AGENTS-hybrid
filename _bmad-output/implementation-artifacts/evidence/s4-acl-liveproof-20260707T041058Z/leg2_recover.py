"""S4 AC-L LEG-2 driver, phase 2: recover the rewound copy at gary@07.

Gary re-enters at §07 against the MUTATED SSOT while CD's retained contribution
attests the ORIGINAL bytes -> genuine same-bytes divergence. In S3 this
WARN-proceeded to G2B. Post-S4 Flip B it must HALT PRE-SPEND: error-pause at
gamma.styleguide.parity-divergence, ZERO Gamma generative spend.
"""
...
try:
    log(f"recover_production_trial({TRIAL2}) with cap=40 — gary re-enters §07 vs MUTATED SSOT "
        "(S4: expect HALT at gamma.styleguide.parity-divergence)")
...
ok = (
    result.get("recover_status") == "paused-at-error"
    and result.get("paused_error_tag") == "gamma.styleguide.parity-divergence"
)
log(f"=== LEG2 PHASE2 {'OK (HALTED as expected)' if ok else 'NOT-OK'} after {result['elapsed_s']:.0f}s ===")
timer.cancel()
