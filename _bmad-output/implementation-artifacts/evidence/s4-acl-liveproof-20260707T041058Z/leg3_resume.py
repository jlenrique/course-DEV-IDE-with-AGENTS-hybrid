"""S4 AC-L LEG-3 driver, phase 2: approve G1, walk to G2B (REAL Gamma deck).

Crosses 4.75 CD (live gpt-5) -> 05/05B irene-pass1 -> 06 builder -> §07 gary
(REAL PAID Gamma deck; matcher fixed at 59a9a48a) -> 7.5 vera -> 07B quinn-r ->
G2B. Anti-outage proof: the S4 flip must NOT break the canonical happy path.
...
state = json.loads((EVIDENCE / "leg3-state.json").read_text(encoding="utf-8"))
TRIAL_ID = UUID(state["trial_id"])
run_dir = RUNS_ROOT / str(TRIAL_ID)
result: dict = {"leg": 3, "phase": "resume", "trial_id": str(TRIAL_ID)}
...
(EVIDENCE / "leg3-resume-result.json").write_text(
    json.dumps(result, indent=2, default=str) + "\n", encoding="utf-8"
)
ok = result.get("resume_status") == "paused-at-gate"
log(f"=== LEG3 PHASE2 {'OK' if ok else 'NOT-OK'} after {result['elapsed_s']:.0f}s ===")
timer.cancel()
