"""S3 AC-L LEG-2 driver, phase 1: rewind-recover PREPARE + SSOT mutation.

Design (recorded for PROOF.md; leg4-udac precedent, driver.py in
evidence/leg4-udac-halt-live-20260701T025814Z):

  1. COPY leg-1's run dir -> a fresh trial id (leg-1 run dir stays untouched
     as leg-1 evidence).
  2. run.json: surgical id rewrite ONLY (no contribution byte edits —
     output_digests are load-validated); DROP gary@07 and every contribution
     at/after manifest index of node 07 so first-wins re-dispatches §07
     onward. CD@4.75's contribution (authored against the ORIGINAL SSOT
     bytes) STAYS — that pins the CD-before mutation ordering.
     Flip status paused-at-gate -> paused-at-error so `trial recover`
     re-enters at the repositioned node.
  3. FABRICATE error-pause.json from the copied G2B checkpoint.json
     (same run_state/runner family the runner itself persists), repositioned
     to node 07 / specialist gary, last_gate_crossed=G1.
  4. Validate the plumbing offline (envelope loads, gary dropped, cd stays,
     directive bytes untouched => all three directive digests stay equal).
  5. ONLY THEN mutate state/config/gamma-style-guides.yaml: ONE
     resolver-emitted field of hil-2026-apc-crossroads-classic
     (prompt_configuration.text_content.amount: minimal -> concise; valid UI
     vocabulary, F-806: alters resolver OUTPUT so ssot_digest AND
     base-resolution digest BOTH diverge). The directive is NEVER touched —
     the disagreement is genuine same-bytes divergence, not drift.

The mutation lands AFTER CD's contribution exists (it was authored in leg 1)
and BEFORE gary dispatches (gary re-dispatches only in phase 2's recover).
"""
...
TAG = "s3.acl.leg2.rewind-reposition-to-gary"
...
leg1 = json.loads((EVIDENCE / "leg1-state.json").read_text(encoding="utf-8"))
TRIAL1 = leg1["trial_id"]
TRIAL2 = str(uuid4())
...
error_pause = {
    ...
    "tag": TAG,
    "message": (
        "recover point repositioned to gary@07 for the S3 AC-L leg-2 "
        "forced-divergence witness (rewind-recover of the leg-1 walk; CD's "
        "contribution retained, gary + downstream dropped)"
    ),
...
# guard: the mutation must land INSIDE the picked guide's block (before the
# next top-level style_guides key)
import re  # noqa: E402
