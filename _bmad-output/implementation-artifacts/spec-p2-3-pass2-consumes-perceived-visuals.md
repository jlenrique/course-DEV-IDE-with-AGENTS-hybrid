...
**AC-6 STRIKE ✅ FIRED (2026-06-20):** all three A6 legs satisfied + independently verified. Operator authorized Claude to run the operator-gated live leg (a) directly via a **fresh independent subagent** under the V1–V4 validity protocol (deterministic committed `detect_fidelity` as sole judge; first-run-stands; no retry-to-green; no edits to detector/fixtures/corpus). A live Pass-2 over the 8-slide green-corpus — fed a *contradicting* stale brief (`$5.2T line+bars` vs perceived `$4.5T`+photo) — produced narration that did NOT carry the stale figure; the committed detector returned **GREEN 8/8** (re-judged independently by the parent) + **held-out green-08 clean**. Cache-MISS confirmed (`cached_tokens=0`, gpt-5 — true cold live call); real prompt incl. un-framed payload tail; cited pre-fix RED baseline = committed RED-first test + Codex A7. **Nuance (recorded honestly):** green-01 narration grounded qualitatively (building photo / stat callouts) without restating `$4.5T`; the load-bearing signal is the absence of the brief's `$5.2T`/`line+bars` + positive perceived grounding (green-08 emitted perceived `12 months`). Deferred entry `fidelity-metric-blind-to-perception-regression` **STRUCK**; bidirectional linkage filed (`docs/trials/cross-trial-learnings.md` §C + §A G5). Evidence: `p2-3-ac6-live-strike-evidence.json`.
