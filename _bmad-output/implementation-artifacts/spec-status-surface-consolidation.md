...
- [ ] **T4 — deferred-inventory archive-section sweep.** ⚠️ **PREMISE CORRECTED (see Change Log):** the `## Closed Entries — Archived` section ALREADY EXISTS (L1051; the audit was wrong). Real remaining work is a hygiene sweep of post-2026-05-07 inline-closed entries. Per the file's OWN maintenance rule this happens at retrospective / multi-slab milestones, not a structural pass — **recommend descope to the next retrospective** (operator to confirm).
