...
- **Gap handling is a core function** (§1.5), **SOURCE+LOs are KING**: derived lesson-level LOs enter as `inferred`/`needs_sme_review`, never silent `source_grounded`.
...
**HOLD-for-operator register (block only the named action, not spec/dev):** (1) PHS 620 slug renames — B-2 gate; (2) legacy-`.doc` long-term policy (extractor vs convert-on-intake) — until decided, `format_unsupported` gap; (3) HAI 510 companion reading-list acquisition; (4) lesson-leaf granularity inside `modules/<slug>/lessons/`; (5) operator LO-ratification surface; (6) first production-target course; (7) gap-ledger committed-vs-evidence disposition (provisional ruling in Story A stands unless overridden); (8) asset kinds beyond the evidence-grounded core.
...
- **A-AC6 (gaps-not-crashes):** empty `sources/` and `lessons/` dirs (the seeded reality) produce gap entries, never crashes — §1.5 exercised for real on day one. The HAI 510 missing companion reading list is emitted as a named gap entry (with an access-coaching note field, per the coaching rule).
...
**Job:** parse both real syllabi into provenance-carrying module-metadata **proposals** (titles/topics/LOs/bucket suggestions) without mutating a single container file. Application of proposals is a separate operator-gated leg (**B-2**, not this story).
