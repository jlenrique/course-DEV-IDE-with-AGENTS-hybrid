...
### ⚠️ CORRECTION (operator, 2026-06-29): Descript publish FAILED — verdict B stands on the LOCAL audio, not a Descript final

The earlier "Descript finals published" claim was OVER-STATED. Operator reports the Descript posting **failed nearly entirely**: the import job had errored (media landed as `type:"other"`, wrong ~6s duration), so although an mp3 appeared to upload, **only one file showed up (nothing to compare) and it would NOT play** in the state Descript left it. The two `share.descript.com` URLs are NOT usable finals.

**Impact on the verdict:** the operator's blind A/B verdict = **B** was made on the **valid LOCAL audio arms** (`evidence/enhanced-vo-ab-20260629/arm-A-no-tag.mp3` vs `arm-B-slow-tag.mp3` — both real, 10.397s, ASR-clean). The finding therefore stands as a **clip-level** A/B (operator ear, first-run-stands). The arc's strict pre-registered "DESCRIPT FINAL video" / full-mix ("opening night") target was **NOT achieved** — it is now an explicit owed cross-confirm, not a closed item.

**Descript-publish DEFECT (next-session investigation):** the MCP direct-upload of the mono 128k MPEG-layer-III mp3 (ID3v2.4) → Descript import errored at the transcode step. Hypotheses to test next: (a) URL import instead of direct upload; (b) convert to WAV/standard MP3 before upload; (c) use the prior-proven `scripts/operator/build_descript_narrated_lesson.py` + `descript_client.py` path (which published successfully in earlier arcs) rather than the claude.ai Descript MCP; (d) the prior known operator item to rotate the Descript token. Produce a proper assembled video (slide+audio), not bare audio compositions.

### Verdict NUANCE (from Codex review-lane operator note) + B-MF-1 — recorded 2026-06-29

**Operator's full reported A/B result (refines the bare "B"):** both arms sounded good as VO; **B was only SLIGHTLY preferable; the perceived difference was INTONATION, not pace; the difference was NOT large and did NOT approach the earlier "confidential tone" audition effect size.** Honest finding: the v3 tag channel is **live and perceptible but SUBTLE** — a *control surface*, NOT sufficient dramatic expressiveness on its own. Do NOT generalize "B" to "v3 tags deliver strong directed VO." The next expressive increment must combine v3 tags with **stronger rhetorical wording / free-form direction / punctuation-prosody / role-specific voice** (the style that produced the much larger "confidential tone" effect in the 27-clip audition), validated in the full Descript mix — not tags alone.

**B-MF-1 (Codex review lane, narrow):** `build_text_channels(canonical, provider)` does NOT itself assert `strip_tags(provider)==canonical` — the byte-exact firewall is enforced by `compile_provider_text` at the production call site, but the channel-builder boundary its docstring calls "build-breaking" is a trusted-internal helper. Not a close blocker (production path is guarded), but filed as a follow-on `enhanced-vo-build-text-channels-firewall-assert`: either add the assertion at `build_text_channels` or document it as trusted-only-behind-`compile_provider_text`.
