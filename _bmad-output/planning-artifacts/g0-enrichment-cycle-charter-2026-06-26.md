...
1. **types every source span** into the closed 10-type enum + `other:<label>` escape hatch (see D1 resolved) — persisted as a typed manifest; types-and-SHOWS, does **not** yet re-wire generation; TYPE is recorded independently of downstream ROLE (`primary/supporting/ignore`);
...
## Open Decisions — ✅ ALL RESOLVED BY OPERATOR (2026-06-26 PM)
- **D1 — Closed TYPE taxonomy: RESOLVED.** Closed enum of **10 types** + an **`other:<label>` ad-hoc escape hatch**:
  `slide / quiz / workbook / narration / reference-citation / rubric / exercise-lab / motion-script-storyboard / discussion-forum / assignment-instructions`.
  - **Escape hatch (operator-ratified):** Marcus-SPOC may record an operator-proposed ad-hoc type as `other:<label>` **with mandatory provenance**, surfaced at the gate flagged as *unconsumed/ad-hoc*; it NEVER silently routes into generation. This preserves A10's closed-enum determinism (the deterministic layer always sees a known shape: a member of the enum OR a flagged `other`) while giving the operator in-conversation "discuss-and-add" flexibility on the upcoming trial.
  - **TYPE ⟂ ROLE (operator clarification, binding):** a span's **TYPE** (what the content IS) is orthogonal to its downstream **ROLE** (`primary / supporting / ignore`). A type with no generator today (`quiz`, `rubric`, `exercise-lab`, `reference-citation`, `discussion-forum`, `assignment-instructions`) is **not dropped** — it is typed accurately AND may be assigned `supporting` (or `ignore`), still feeding lesson planning or ad-hoc use (e.g. content folded into a workbook). A10's "flag types with no consumer" means *flag for generation-routing*, NOT exclude from the corpus. Consumer map today: HAS consumer = `slide` / `workbook` (07W) / `narration` / `motion-script-storyboard` (07D.5→kira→storyboard); classification-only-flagged = the other six.
- **D2 — "Complete source": RESOLVED = operator confirms the manifest Marcus presents.** NO expected-source inventory in v1. Marcus never asserts "complete," only "here's what I reached"; completeness is the operator's call at the gate. (Reliable-access half stays mechanical/deterministic + RED-on-failure per A1.)
- **D3 — v1 cut: RESOLVED = build-then-trial** (type + LO-with-provenance + Marcus-owned confirm + completeness-hard-assert + **the REQUIRED Irene refinement loop** per the operator override; generation-routing DEFERRED).
- **D4 — Irene in the room: RESOLVED = include Irene.** Next party round convenes WITH Irene to sign the **LO-delta contract** (refined LOs + source-tied rationale diff + source-adequacy report) and ratify the unified LO schema (A7) before stories open.
