...
8. ~~**Raised success bar** (Murat+Mary): 3 CONSECUTIVE clean runs + 1 CROSS-DECK run.~~ **RETIRED by operator 2026-06-24 PM.** The operator has no interest in a formal 3-consecutive + cross-deck reliability GATE: "we will inevitably do such checks as we do real production runs going forward with our production run/fix/run/fix cycles." Reliability is now proven ORGANICALLY through real production cycles, not an upfront gate. (The per-number honesty discipline still holds: ghost-count=0 figure-contradiction + resubstitution stamps + no generalization claims; that's separate from the retired run-count bar.)
