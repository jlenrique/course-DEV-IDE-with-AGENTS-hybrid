...
| **AC-2a.4-B-OP live retrieval-dispatch reactivation (Texas)** (`2a.4-followon-ac-b-op-live-retrieval`) | Story 2a.4 pre-T1 party-mode round 2026-04-25 (3-1 consensus: Winston + Murat + Paige → Path 3 defer; Amelia → Path 4 fallback compatible with defer) | `skills/bmad-agent-texas/scripts/run_wrangler.py:58` imports `marcus.dispatch.contract` which does not exist on hybrid (`marcus/dispatch/` absent — only `intake/lesson_plan/orchestrator/facade.py` shipped). Empirical grep gate (`grep -rn "marcus\.dispatch" skills/bmad-agent-texas/`) found marcus.dispatch.contract symbols **consumed on the runtime path** at `run_wrangler.py:120-125 _to_dispatch_outcome()` + `:1707-1709 build_dispatch_envelope(DispatchKind.TEXAS_RETRIEVAL)` + `:1717 build_dispatch_receipt()` + `:2021-2031 second-invocation-site` — NOT dead-on-arrival as initially theorized. Lazy-import shim (Path 2) would silently produce broken envelope shape → vacuous-pass evidence (Murat M5 binding from 2a.3 P9 lesson). Forward-port marcus.dispatch.contract (Path 1, ~50-100 LOC) was rejected as Slab-3 architectural scope drift inside a scaffold-migration story. Defer (Path 3) is the disciplined call. **Mock-vs-live delta accepted at 2a.4** (Murat hard caveat point a): AC-B mockable wrapper shape verification is good but NOT live-wire proven — sha256 baselines (M2 sanctum lock + M3 NFR-I5 retrieval-contract) ARE live-proven (real files); subprocess-dispatch-latency (M4) is NOT measurable until live wire fires; bundle-parser parametrized parse-branch coverage (M1, 7 cases) is mock-proven against fixture bundle. **Deferral authority (Murat hard caveat point b):** Story 2a.4 pre-T1 party-mode round 2026-04-25 (3-1 consensus); see Story 2a.4 spec §"Filled-in evidence" Completion-Notes section + this inventory entry. | **Slab-3 reactivation gate (Murat hard caveat point c — BINDING; Slab-3 close cannot accept "AC-B-OP fires" alone):** before Slab-3 closes, re-execute the AC-B-OP path on **live wire** with the full M1-M5 test discipline applied: (i) 7 parametrized parse-branch cases against actual scite/Consensus MCP responses (not fixture bundles); (ii) two-sided assertions on parse outcome AND resolution-trail tag; (iii) M2/M3 sha256 baselines re-validated post-Slab-3-changes (sanctum lock + NFR-I5 retrieval-contract preservation); (iv) M4 subprocess-dispatch-latency p50/p95 measured + recorded in §12.10 retrospective as the substitute-metric baseline; (v) operator runs `.venv/Scripts/python.exe scripts/utilities/ac_b_op_texas_live_retrieval_evidence.py --directive tests/fixtures/specialists/texas/ac_b_op_directive.yaml --bundle-dir <path>` end-to-end + pastes filled-in evidence Markdown block into the Slab-3 reactivation story. Helper script + directive template are PRE-AUTHORED at 2a.4 and SHIP UNINVOKABLE-ON-HYBRID-TODAY; Slab-3 inherits without re-authoring. Anti-pattern harvest (Paige proposal at party-mode 2026-04-25): file as **A13 — "Premature live-wire AC against not-yet-ported substrate"** OR augment A9 (structural-name drift) sibling subsection at Slab-3 reactivation. The harvest is BETTER than codifying a lazy-import shim pattern (which would invite 14 Slab-2b specialists to reach for it reflexively). |
...
| **`SanctumLockViolation` cross-specialist refactor** (`slab-2a-close-followon-sanctum-lock-cross-cutting`) | Story 2a.4 G6 close (2026-04-25) | 2a.4 introduced `SanctumLockViolation(RuntimeError)` named exception class for Texas with hard sha256 lock-and-verify protocol per Winston W1 rider; Irene (2a.2) and Kira (2a.3) currently raise generic `RuntimeError` on lock drift. Cross-specialist consistency would let any specialist's lock-and-verify tests assert the same exception class. | **Slab-2-cross-cutting hygiene, ~0.5pt.** Promote `SanctumLockViolation` to a shared module (e.g., `app.specialists._scaffold.sanctum.exceptions`) and migrate Irene/Kira/Texas to use it. Fire-trigger: when any 2b specialist's spec references the lock-and-verify protocol; shared exception location is more discoverable than per-specialist re-derivation. Filed at 2a.4 close. |
| **§12 size review when subsection count exceeds 12** (`slab-2a-close-followon-section-12-size-review`) | Story 2a.4 close + Slab 2a retrospective (2026-04-25, Paige P8 rider) | `docs/dev-guide/langgraph-migration-guide.md §12 Specialist Walkthrough` currently has 10 subsections (§12.1–§12.10) post-2a.4. Slab 2b adds ~14 specialist migrations across 14 stories; §12 grows by 0 to 14 worked-example subsections depending on how the walkthrough convention is applied per category. | **Readability check at 2b mid-slab, ~0pt (operator decision).** When subsection count crosses 12, re-evaluate whether §12 should be split into a directory (`docs/dev-guide/migration-walkthroughs/{category}/{specialist}.md`) or stay as a single section. Fire-trigger: §12 subsection count ≥ 13. Paige P8 rider 2026-04-25 ratified. |
...
| **Sanctum lock baseline regeneration script** (`slab-2a-close-followon-sanctum-lock-regen`) | Story 2a.4 G6 close (2026-04-25) | Texas's 17-file sha256 baseline ships as `TEXAS_SANCTUM_LOCK_BASELINE` module-level constant per Amelia A3 rider; operator can manually regenerate via `find _bmad/memory/bmad-agent-texas -type f -exec sha256sum {} +` per Story §G when sanctum legitimately changes. A regen script (~30 LOC) automates this for any Slab-2 specialist with a populated-and-locked sanctum. | **Operator-helper, ~0.25pt.** Author `scripts/utilities/regenerate_sanctum_lock_baseline.py <specialist-name>` that emits the dict literal in the exact `TEXAS_SANCTUM_LOCK_BASELINE` shape ready to paste into the specialist's graph.py. Fire-trigger: any Slab-2b specialist that ships with a populated-and-locked sanctum + operator session where a baseline regen is needed. Filed at 2a.4 close. |

**Total named follow-ons: 23.** (Four new entries added 2026-04-24 from Story 2a.2 T2 + T4 + T7 + T8 party-mode waypoint reviews; one new entry 2026-04-25 from Story 2a.4 pre-T1 party-mode round — AC-B-OP live retrieval-dispatch reactivation pending Slab-3 marcus.dispatch.contract forward-port; six new entries 2026-04-25 from Story 2a.4 G6 close + Slab 2a retrospective convening — dispatch-wrapper extraction candidate / SanctumLockViolation cross-specialist refactor / §12 size review / implementation-artifacts README / local env optional-dep hygiene / sanctum lock regen script; one entry closed at 2a.5.)
...
| Named-but-not-filed follow-ons | 12 | Triggered by parent-story closure + condition |
