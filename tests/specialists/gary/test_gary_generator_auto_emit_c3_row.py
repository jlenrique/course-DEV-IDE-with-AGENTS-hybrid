...
def test_gary_dispatch_wrapper_module_exists_and_imports() -> None:
    from app.specialists.gary.gamma_dispatch import dispatch_to_gamma

    assert callable(dispatch_to_gamma)


def test_post_emit_temp_pyproject_has_gary_row_and_no_duplicate(
    temp_repo_root: Path, make_request
) -> None:
    gary_skill = temp_repo_root / "skills" / "bmad-agent-gamma"
    gary_skill.mkdir(parents=True, exist_ok=True)
    (gary_skill / "SKILL.md").write_text("# gary skill", encoding="utf-8")
    ...
    generate.generate_specialist(
        make_request(
            ...
            from_skill=gary_skill,
            repo_root=temp_repo_root,
            force=True,
        )
    )
    pyproject = _read_pyproject(temp_repo_root)
    target = "app.specialists.gary.graph -> app.gates.resume_api"
    rows = _c3_ignore_imports(pyproject)
    assert target in rows
    assert rows.count(target) == 1
    assert '# generated by bmad-create-specialist for gary' in pyproject
