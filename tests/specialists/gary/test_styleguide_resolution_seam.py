...
def test_classic_styleguide_expands_to_api_keys() -> None:
    out = _normalized_gamma_settings(_settings(styleguide="hil-2026-apc-crossroads-classic"))
...
def test_per_variant_override_beats_styleguide_base() -> None:
    out = _normalized_gamma_settings(
        _settings(styleguide="hil-2026-apc-crossroads-classic", dimensions="16x9")
    )
    a = next(item for item in out if item["variant_id"] == "A")
...
def test_source_keywords_win_over_styleguide_base() -> None:
    ...
    out = _normalized_gamma_settings(
        _settings(styleguide="hil-2026-apc-crossroads-classic", keywords=source_keywords)
    )
    a = next(item for item in out if item["variant_id"] == "A")
...
def test_empty_keywords_list_does_not_clobber_styleguide() -> None:
    ...
    out = _normalized_gamma_settings(_settings(styleguide="videographic-glance-track", keywords=[]))
    a = next(item for item in out if item["variant_id"] == "A")
    assert a["keywords"] == ["vector", "minimalist", "flat-color", "single-focal-icon", "bold"]
