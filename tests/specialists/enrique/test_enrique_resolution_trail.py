...
from app.specialists.enrique.graph import _plan


def test_enrique_plan_appends_resolution_entry_to_trail() -> None:
...
def test_enrique_chat_metadata_contains_span_fields() -> None:
    handle = make_chat_model(specialist_id="enrique", tier_request="reasoning")
    metadata = getattr(handle.chat, "metadata", {}) or {}
    assert metadata.get("specialist_id") == "enrique"
