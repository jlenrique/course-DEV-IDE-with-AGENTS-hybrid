from __future__ import annotations

import json
from pathlib import Path

from app.manifest.compiler import SPECIALIST_ALIASES
from app.models.state import specialist_summary_artifacts as summary_writer
from app.models.state.cache_state import CacheState
from app.models.state.run_state import RunState
from app.specialists.enrique.graph import _emit_spans


def test_enrique_summary_lands_via_7a5_facade_and_alias(tmp_path: Path, monkeypatch) -> None:
    original_emit = summary_writer.emit_summary_for_state

    def _emit_with_tmp_root(
        specialist_id: str, state: RunState, *, gate_id: str = "specialist-complete"
    ) -> dict[str, object]:
        return original_emit(specialist_id, state, gate_id=gate_id, runs_root=tmp_path)

    monkeypatch.setattr(
        "app.specialists.enrique.graph.specialist_summary_writer.emit_summary_for_state",
        _emit_with_tmp_root,
    )
    state = RunState(
        graph_version="v0.1-stub",
        cache_state=CacheState(cache_prefix=json.dumps({"narration_outputs": []})),
    )
    assert SPECIALIST_ALIASES["elevenlabs"] == "enrique"
    assert summary_writer.canonical_specialist_id("elevenlabs") == "enrique"
    _emit_spans(state)
    summary_dir = tmp_path / str(state.run_id) / "specialist-summaries"
    summaries = list(summary_dir.glob("enrique-*.md"))
    assert len(summaries) == 1
    text = summaries[0].read_text(encoding="utf-8")
    assert "Enrique" in text
    assert "G2" in text
