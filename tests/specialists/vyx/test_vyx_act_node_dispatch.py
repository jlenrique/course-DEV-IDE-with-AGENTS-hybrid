...
from app.specialists.vyx.graph import (
...
def test_vyx_parse_envelope_happy_path() -> None:
    parsed = _parse_operator_instructions_envelope({"prompt": "Draft operator steps"})
    assert parsed["tag"] == "storyboard.parsed.ok"


@pytest.mark.parametrize(
    "payload,tag",
    [
        ({}, "storyboard.parsed.missing-key"),
        ({"prompt": 7}, "storyboard.parsed.wrong-type"),
        ({"prompt": "ok", "context": "bad"}, "storyboard.parsed.wrong-type"),
    ],
)
def test_vyx_parse_envelope_errors(payload: object, tag: str) -> None:
...
@pytest.mark.parametrize(
    "cache_prefix,tag",
    [
        ("{bad", "storyboard.parsed.malformed"),
        ("[]", "storyboard.parsed.wrong-type"),
        (json.dumps({}), "storyboard.parsed.missing-key"),
        (json.dumps({"prompt": 7}), "storyboard.parsed.wrong-type"),
    ],
)
def test_vyx_act_error_branches_append_trail_tag(cache_prefix: str, tag: str) -> None:
...
def test_vyx_act_missing_cache_state_appends_trail_tag() -> None:
    state = RunState(
        graph_version="v0.1-stub",
        temperature=0.0,
        model_resolution_trail=[
    ...
    with pytest.raises(OperatorInstructionsParseError) as exc_info:
        _act(state)
    assert exc_info.value.tag == "storyboard.parsed.missing-key"
    assert state.model_resolution_trail[-1].reason == "storyboard.parsed.missing-key"


def test_vyx_act_returns_deterministic_payload() -> None:
