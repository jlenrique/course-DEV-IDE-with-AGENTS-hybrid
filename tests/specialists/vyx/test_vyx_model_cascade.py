...
def test_vyx_default_resolution_resolves_to_gpt_5_4() -> None:
    handle = make_chat_model(specialist_id="vyx", tier_request="fast")
    assert handle.entry.resolved == "gpt-5.4"
    assert handle.entry.level == "per_specialist"


def test_vyx_per_call_override_wins() -> None:
