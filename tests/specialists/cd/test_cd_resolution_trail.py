...
from app.specialists.cd.graph import _plan


def test_cd_plan_appends_resolution_entry_to_trail() -> None:
...
def test_cd_chat_metadata_contains_span_fields() -> None:
    handle = make_chat_model(specialist_id="cd", tier_request="fast")
    metadata = getattr(handle.chat, "metadata", {}) or {}
    assert metadata.get("specialist_id") == "cd"
