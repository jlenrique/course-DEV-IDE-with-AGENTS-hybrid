...
from app.specialists.wanda.graph import _plan


def test_wanda_plan_appends_resolution_entry_to_trail() -> None:
...
def test_wanda_chat_metadata_contains_span_fields() -> None:
    handle = make_chat_model(specialist_id="wanda", tier_request="reasoning")
    metadata = getattr(handle.chat, "metadata", {}) or {}
    assert metadata.get("specialist_id") == "wanda"
