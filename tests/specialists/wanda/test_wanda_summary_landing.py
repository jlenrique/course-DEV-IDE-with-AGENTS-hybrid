...
from app.specialists.wanda.graph import _emit_spans


def test_wanda_summary_lands_via_7a5_facade(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "app.specialists.wanda.graph.specialist_summary_writer.emit_summary_for_state",
        _emit_with_tmp_root,
    )
    state = RunState(
        graph_version="v0.1-stub",
        cache_state=CacheState(cache_prefix=json.dumps({"audio_beds": []})),
    )
    _emit_spans(state)
    summary_dir = tmp_path / str(state.run_id) / "specialist-summaries"
    summaries = list(summary_dir.glob("wanda-*.md"))
    assert len(summaries) == 1
    text = summaries[0].read_text(encoding="utf-8")
    assert "Wanda" in text
    assert "G2" in text
