...
class TestGaryActivationContract(SanctumParityTestBase):
    ...
    def test_gamma_api_client_binding(self) -> None:
        source = (REPO_ROOT / "app" / "specialists" / "gary" / "_act.py").read_text(
            encoding="utf-8"
        )
        client_source = (REPO_ROOT / "scripts" / "api_clients" / "gamma_client.py").read_text(
            encoding="utf-8"
        )
        assert "GammaClient" in source
        assert ".generate_deck(" in source
        assert "def generate_deck(" in client_source
        assert "requests." not in source

    def test_vcr_cassettes_present(self) -> None:
        cassette_dir = REPO_ROOT / "tests" / "fixtures" / "specialist-replay" / "gary"
        assert (cassette_dir / "gamma_dispatch_happy_path.yaml").is_file()
        assert (cassette_dir / "gamma_dispatch_double_dispatch.yaml").is_file()
    ...
    def test_credential_rotation_register_entry(self) -> None:
        ...
        gamma = next(row for row in rows if row["provider"] == "gamma")
        assert gamma["owner"] == "operator"
        assert gamma["rotation_cadence_days"] == 90
        assert gamma["secret_store_reference"] == "operator-machine .env GAMMA_API_KEY"

    def test_rate_limit_budget_declared(self) -> None:
        path = REPO_ROOT / "app" / "specialists" / "gary" / "config.yaml"
        data = yaml.safe_load(path.read_text(encoding="utf-8"))
    ...
    def test_cache_hit_rate_not_applicable(self) -> None:
        source = (REPO_ROOT / "app" / "specialists" / "gary" / "model_config.yaml").read_text(
            encoding="utf-8"
        )
        assert "REST-API tool-dispatch" in source
        assert not (REPO_ROOT / "tests" / "end_to_end" / "test_gary_cache_hit_rate.py").exists()
    ...
    def test_operator_gated_canary_documented(self) -> None:
        spec = (
            ...
            / "migration-7b-6-gary-port-shape.md"
        ).read_text(encoding="utf-8")
        assert "Operator-gated AC-6-B" in spec
        assert "cost" in spec
