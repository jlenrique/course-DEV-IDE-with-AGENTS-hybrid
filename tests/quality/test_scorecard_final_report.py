...
def test_real_did_leaks_reconcile_with_open_leaks() -> None:
    ...
    coverage_honesty (1) + fidelity_trust (1) + capability_honesty (1) = 9 (GL-13; Q2.1
    added cost_efficiency's ``leaks`` list, Q2.2 added coverage_honesty's, Q2.3 added
    fidelity_trust's, Q3.1 added capability_honesty's)."""
    block = read_scorecard_block()
    ...
    assert total_open == 9
    assert len(ranked_project_leaks(block)) == total_open == 9
