...
def test_real_did_leaks_reconcile_with_open_leaks() -> None:
    ...
    = 11 (GL-13; Q2.1 added cost_efficiency's ``leaks`` list, Q2.2 added coverage_honesty's,
    Q2.3 added fidelity_trust's, Q3.1 added capability_honesty's, Q3.2 added tracker_coherence's
    TWO governance leaks — TC1 structural divergence + TC2 advisory-not-gating monitor)."""
    block = read_scorecard_block()
    ...
    assert total_open == 11
    assert len(ranked_project_leaks(block)) == total_open == 11
