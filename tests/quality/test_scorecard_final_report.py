...
def test_real_did_leaks_reconcile_with_open_leaks() -> None:
    ...
    coverage_honesty (1) = 7 (GL-13; Q2.1 added cost_efficiency's ``leaks`` list, Q2.2
    added coverage_honesty's)."""
    block = read_scorecard_block()
    dim = block["dimensions"][_DID_KEY]
    assert len(dim["leaks"]) == dim["open_leaks"] == 5
    # Ranked list = sum of every dimension's open_leaks (DID 5 + cost 1 + coverage 1).
    total_open = sum(
        d.get("open_leaks", 0) for d in block["dimensions"].values() if isinstance(d, dict)
    )
    assert total_open == 7
    assert len(ranked_project_leaks(block)) == total_open == 7
