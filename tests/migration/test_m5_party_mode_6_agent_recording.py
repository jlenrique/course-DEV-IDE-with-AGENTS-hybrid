...
AGENTS = ("Winston", "Murat", "Paige", "Quinn-R", "Amelia", "Dr. Quinn")
PER_AGENT = r"SHIP|ITERATE|ROLLBACK|SHIP-WITH-RIDERS|SHIP-CONDITIONAL|ABSTAIN"
...
def test_m5_party_mode_6_agent_recording() -> None:
    text = VERDICT.read_text(encoding="utf-8")
    party = re.search(
        r"^## Party-Mode Verdict \(6 agents\)\n(?P<body>.*)",
        text,
        flags=re.MULTILINE | re.DOTALL,
    )
