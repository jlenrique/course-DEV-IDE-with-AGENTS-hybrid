...
def test_slab_3_retrospective_exists_with_required_sections_and_verdicts() -> None:
    retrospective_path = Path("_bmad-output/implementation-artifacts/slab-3-retrospective.md")
    ...
    verdicts = re.findall(
        r"[23][a-z0-9\.-]*[-a-z0-9]+:\s*"
        r"(?:RESOLVED|DEFERRED-CONTINUES|REACTIVATED-AT-SLAB-X|NOT-APPLICABLE)",
        next_slab_section.group("body"),
    )
    assert len(verdicts) >= 3
    assert "3-6-marcus-envelope-baseline-capture-for-future-regression-detection: RESOLVED" in (
        next_slab_section.group("body")
    )
