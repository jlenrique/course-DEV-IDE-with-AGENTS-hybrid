...
def test_projection_first_get_is_200_with_seq_etag(client: TestClient) -> None:
...
def test_unknown_schema_version_serves_raw_with_unrecognized_etag(
    ...
    app = create_hud_app(str(bound_trial_id), run_dir, "n", "session")
    resp = TestClient(app).get("/projection")
    # Unrecognized snapshots have no identity to guard and are served raw so the
    # HUD can render UNRECOGNIZED literally (never coerced).
    assert resp.status_code == 200
