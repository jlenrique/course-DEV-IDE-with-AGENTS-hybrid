...
def test_migrated_manifest_has_33_nodes() -> None:
...
def test_migrated_manifest_edges_form_linear_chain() -> None:
    ...
    manifest = load(MIGRATED_V42_MANIFEST_PATH)
    # 33 nodes + 2 sentinel edges (START→01, 15→END) = 34 edges total.
    assert len(manifest.edges) == len(manifest.nodes) + 1  # +1 for __start__ entry
