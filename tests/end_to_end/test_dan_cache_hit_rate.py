...
from app.specialists.dan.graph import _act, _plan
...
def test_dan_cache_hit_rate_post_warmup() -> None:
    if os.environ.get("DAN_LLM_LIVE_CACHE_HARNESS") != "1":
        pytest.skip(
            "operator-gated: set DAN_LLM_LIVE_CACHE_HARNESS=1 to spend live "
            "OpenAI calls on Dan cache-hit-rate measurement"
        )
    payload = {
        "topic": "renal physiology",
        "lesson_outline": "learner tension and visual sequence " * 900,
    }
    ratios = []
    for _ in range(10):
        state = RunState(
