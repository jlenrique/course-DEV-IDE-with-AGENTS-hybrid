...
POLL_SURFACE = Path("app/gates/section_07c/poll_surface.py")
MODULE_NAME = "app.gates.section_07c.poll_surface"
...
def test_section_07c_surface_has_one_module_level_parity_contract() -> None:
...
def test_section_07c_registers_declared_transports_without_alias() -> None:
    _reload_poll_surface_registration()

    surfaces = {surface.surface_id: surface for surface in iter_registered_surfaces()}
    declaration = surfaces["section_07c_storyboard_build"]
...
def test_self_registration_audit_passes_for_section_07c(tmp_path: Path) -> None:
    _reload_poll_surface_registration()

    result = run_self_registration_audit(
        declared_floor=1,
        discovery_roots=("app.gates.section_07c",),
