...
POLL_SURFACE = Path("app/gates/section_04a/poll_surface.py")
MODULE_NAME = "app.gates.section_04a.poll_surface"
...
def test_poll_surface_registers_g1_alias_and_declared_transports() -> None:
    _reload_poll_surface_registration()

    surfaces = {surface.surface_id: surface for surface in iter_registered_surfaces()}
    declaration = surfaces["section_04a_g1a_poll"]
...
def test_self_registration_audit_passes_with_section_04a_root(tmp_path: Path) -> None:
    _reload_poll_surface_registration()

    result = run_self_registration_audit(
        declared_floor=1,
        discovery_roots=("app.gates.section_04a",),
