...
POLL_SURFACE = Path("app/gates/section_05_5/poll_surface.py")
MODULE_NAME = "app.gates.section_05_5.poll_surface"
...
def test_g2b_surface_has_exactly_one_module_level_parity_contract() -> None:
...
def test_g2b_surface_registers_declared_transports_and_alias() -> None:
    _reload_poll_surface_registration()

    surfaces = {surface.surface_id: surface for surface in iter_registered_surfaces()}
    declaration = surfaces["section_05_5_g2b_per_slide_mode"]
...
def test_self_registration_audit_passes_for_section_05_5(tmp_path: Path) -> None:
    _reload_poll_surface_registration()

    result = run_self_registration_audit(
        declared_floor=1,
        discovery_roots=("app.gates.section_05_5",),
