...
POLL_SURFACE = Path("app/gates/section_04_5/poll_surface.py")
MODULE_NAME = "app.gates.section_04_5.poll_surface"
...
def test_estimator_surface_has_exactly_one_module_level_parity_contract() -> None:
...
def test_estimator_surface_registers_mandatory_and_optional_transports() -> None:
    _reload_poll_surface_registration()

    surfaces = {surface.surface_id: surface for surface in iter_registered_surfaces()}

    declaration = surfaces["section_04_5_g1_5_estimator"]
...
def test_self_registration_audit_passes_with_section_04_5(tmp_path: Path) -> None:
    _reload_poll_surface_registration()

    result = run_self_registration_audit(
        declared_floor=1,
        discovery_roots=("app.gates.section_04_5",),
