...
## NEXT — dispatch Codex on S2
**S2 is pre-staged + dispatch-ready:** `codex-dev-prompt-p2-4c-s2.md` (per-element image-role tier emission; G2 resolved — perceiver-primary + geometry gate/backfill; scored tiers {1,2,4}, 2.5→2 fold, tier-3 quarantine, κ≥0.6 + confusion-matrix). Operator dispatches Codex (T1–T10) → Claude T11. Then S3 (`codex-dev-prompt-p2-4c-s3.md`, escalation) → T11 → **P2-4b finalize** (score the built classifier vs the operator-confirmed held-out labels). Codex prior-cycle hand-back precedent: `p2-4c-s1-t11-code-review-2026-06-23.md`.
