...
**▶ E2E RESULT (fresh trial `be075675`, AFTER the gap-#4 fix):** the new pipeline ran **error-free, live, end-to-end through its own gates** — G0E (live enrichment, paused→approved) → G0R (live Irene refinement, paused→approved) → **LOs RATIFIED** (`ratified-los.json` written) → continued into the real production pipeline (node 01 → node 02). It then hit `bundle.dispatch.input-missing: dispatch_retrieval missing 'directive_path'` at **node 02 (Texas)** — the KNOWN PRE-EXISTING baseline red (`test_texas_to_cd_chain`, the Texas directive_path dispatch gap), NOT this work, and well upstream of Gary (node 06). **So the G0-enrichment + Irene-refinement + ratify flow is E2E-PROVEN clean; the path to Gary is blocked by a pre-existing Texas dispatch bug.**

**What is next:** (1) fix the pre-existing Texas `directive_path` gap (node 02) — that's the next blocker to reach Gary; the run setup (`run_production_trial` from a corpus dir) doesn't thread a directive_path Texas's `dispatch_retrieval` requires. Then the run continues toward Gary. (2) Re-run to confirm G0→...→hand-off-to-Gary clean ×2. (3) Build **P2 (Texas pass-0) / P3 (Irene pass-1) / P4 (materializer + gate knob)** per the charter. (4) 3-layer T11 on S2/S3/P1 (owed, batched); record resolved model_id (drop the `marcus` pricing alias). Use `scratchpad/e2e_full.py` (FRESH trial each run) + the live-key recipe.
