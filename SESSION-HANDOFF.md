...
**Open / deferred (deferred-inventory, all with triggers):** `fidelity-L1-per-slide-text-mode` (trigger: prose-heavy low-numeric module); fidelity semantic leg (≥3 runs); `clustering-1.3-control-keep-dense-pin` (trigger: organic chunk/keep-dense destabilizes over 2-3 trials); `clustering-1.3-timing`; `gary-export-llm-brief-to-page-matcher` (operator-raised, replace deterministic fuzzy title-match with LLM); `desmond-delivery-sanctum-lock-vs-operator-wip` (operator will baseline). **Open operator items:** rotate the Descript token; commit/baseline the Desmond sanctum WIP; the raised reliability bar (3-consecutive + cross-deck) still banked.
