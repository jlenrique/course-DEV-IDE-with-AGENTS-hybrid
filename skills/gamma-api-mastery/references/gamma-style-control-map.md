...
| **Card headers & footers** · 6 positions, text/image/cardNumber (Pro) | `cardOptions.headerFooter` · positions + element types | — **NOT emitted** | CD-registry `page_settings.card_options.card_headers_footers` | **NONE** (API supports it; Gary doesn't wire it) | ❌ dropped |
...
| **Export as** · PDF / PPTX / PNG | `exportAs` · `pdf`,`pptx`,`png` | **Gary-HARDCODED** `"png"` — `_act.py:962,854` | Gary-hardcoded | Gamma API | ✅ (registry `page_settings.export.export_as` is **decorative** — never read) |
...
**B. Registry fields authored but consumed by NO ONE (decorative — prune or explicitly label):** `page_settings.card_options.{card_width, content_alignment, font_size, show_card_backdrops, animations, card_headers_footers}`, `page_settings.export.export_as`, `studio_prompt_lock.{wrapper_id,behavior,guard}`, `coherence.*`, `provenance.*`. (`presentation.*` is picker-display-only by design — not decorative.)
