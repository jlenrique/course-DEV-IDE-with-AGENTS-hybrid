...
## Marcus-SPOC Runtime — Implementation Seams & Boundaries

> **Current product status / where-we-are lives in [`STATE-OF-THE-APP.md`](STATE-OF-THE-APP.md) §11 (the product-truth SSOT), not here.** This guide is anchored on the Marcus-SPOC local runtime — the operator-facing APP orchestrator + its deterministic production runtime; proofing/concierge sessions are discovery vehicles only. The seam maps below are durable developer reference — when a status detail drifts from SOTA §11, SOTA wins.
...
- **Dependency installs go through pip, not uv:** `managed = false` also disables `uv sync`/`uv add` for this repo — install deps only via `.venv/Scripts/python.exe -m pip install -e ".[dev]"`. On a machine without uv, install it first (`winget install astral-sh.uv`) — v6.10+ skill scripts invoke `uv run` internally. After any installer re-run, check `git status _bmad/custom/` and commit or revert before invoking `bmad-dev-auto` (its clean-tree gate HALTs on dirt).
- **The repo carries no tracked pin of its BMAD version** (`_bmad/*` is gitignored except `memory/` sanctums and `custom/` overrides). The upgrade-record artifact is the only cross-machine signal; after pulling, re-run the installer per machine, pinned to the recorded version so machines converge: `npx bmad-method@6.10.0 install --directory . --action update --all-stable -y`.
...
> Legacy migration context archived to [`dev-guide.history.md`](dev-guide.history.md) (pre-migration workflow + migration-status banners). Current product status → [`STATE-OF-THE-APP.md`](STATE-OF-THE-APP.md) §11.
