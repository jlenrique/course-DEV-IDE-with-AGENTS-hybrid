...
> **Trial-3 context (pre-fill notes):** First tracked trial post-Slab-7c. FRESH corpus per operator 2026-05-07 (NO Tejal-APC-C1 carry-forward). Substrate state: post-S2 marcus/ namespace collapse landed at commit `e48e107`. Methodology per `docs/trials/methodology.md`. Pre-launch: read `cross-trial-learnings.md` (5 min, mandatory).


> **This is the trial-3 skeleton; fill in place.** Originally copied from `docs/trials/trial-N-templates/launch.md` BEFORE you launch Trial-N. Fill placeholders. Read the EXAMPLE block at the top to anchor the shape; then fill the EMPTY block below it. Reads in 5 minutes; fills in 10-15 minutes.
