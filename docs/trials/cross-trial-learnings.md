...
- **Vision perception went GENUINELY LIVE (2026-06-21, `vision-perceiver-real` enabler).** Discovered at P2-4b kit prep that the "committed" vision perceiver was a **fixture-backed contract** — `app/specialists/vision/provider.py` POSTed to an unconfigured `VISION_PROVIDER_ENDPOINT`, default model `vision-fixture-v1`, only a hand-authored slide-01 golden on disk; P2-2 had closed "real PerceptionArtifact" but no live model was wired. Rewired to a real live `gpt-5.5` multimodal call (house `ChatOpenAI`/cascade); live AC-8 perceived all 6 frozen-corpus PNGs HIGH/perceived (slide-01 anchors $4.5T/74%/3x present), key-gated, first-run-stands. Party green-light 5/5 → 3-layer code-review → remediation (M1: the catalog-snapshot test was ALSO RED for `gpt-5.4` on clean HEAD — a second stale tracker) → 4/4 CLOSE. **Systemic-drift pattern** (a green suite standing in for live capability / current config) → anti-pattern G1; reactivation sweep filed as deferred-inventory `believed-green-tracker-audit`. Bidirectional → that inventory entry.
