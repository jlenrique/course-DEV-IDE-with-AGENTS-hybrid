...
| **Texas (Source Wrangler)** | `skills/bmad-agent-texas/` | Source extraction with quality validation, cross-validation against reference assets, fallback chains, proportionality checks. Memory agent (evolvable). Replaces legacy `skills/source-wrangler/`. **Runtime entry point**: `python skills/bmad-agent-texas/scripts/run_wrangler.py --directive <path> --bundle-dir <path>` — orchestrates fetch + extract + validate + cross-validate + emits 6 bundle artifacts per `references/extraction-report-schema.md` (Epic 25 Story 25-1, 2026-04-17). **2026-04-18 (Story 27-0)**: Shape 3-Disciplined retrieval foundation package shipped at `scripts/retrieval/` (contracts / base ABC / dispatcher / hand-rolled MCP client / provider directory / FakeProvider). Operator directory surface: `python skills/bmad-agent-texas/scripts/run_wrangler.py --list-providers [--shape retrieval\|locator] [--status ready\|stub\|ratified\|backlog] [--json]` — canonical answer to "what can Texas fetch?" See `references/retrieval-contract.md` (audience-segmented — Tracy / operators / dev-agents) + `_bmad-output/implementation-artifacts/SCHEMA_CHANGELOG.md` for schema v1.1. Auth contracts: **scite MCP requires OAuth 2.0 Bearer** (verified live 2026-06-25; `api.scite.ai/mcp` advertises authorization_code + refresh_token grants, PKCE S256 — NOT HTTP Basic; `SCITE_USER_NAME`/`SCITE_PASSWORD` feed the autonomous login tool, not sent as Basic). One-time sign-in via `scripts/operator/scite_oauth_login_auto.py` (Playwright, headed; creds from `.env`) → token at `secrets/scite_oauth_token.json` (gitignored); `retrieval/scite_oauth_token.load_bearer_token()` refreshes headlessly thereafter. **Live search tool is `search_literature`** (result wrapped in MCP `content[0].text`→`hits`). Consensus MCP accepts `CONSENSUS_API_KEY` (bearer) or `CONSENSUS_USER_NAME` + `CONSENSUS_PASSWORD` (default URL `https://mcp.consensus.app/mcp`). Scite live-proven (3 real cited TexasRows, 2026-06-25); Consensus pending `CONSENSUS_API_KEY`. **2026-07-02 (Leg-E)**: NEW retrieval provider **`gamma_docs`** (`scripts/retrieval/gamma_docs_provider.py`, status `ready`, live-proven) — fetches developers.gamma.app raw markdown (`.md` endpoints / `llms.txt`); **doc-audit tooling, NOT a course-content retrieval source** (see PROVIDER_INFO.notes). Consumed by the audit driver `scripts/utilities/audit_gamma_docs.py` (3-terminal-state doc-vs-code drift audit; writes candidate observations to `state/config/gamma-learned-observations.jsonl`; exit 0/10/20). | Active |
