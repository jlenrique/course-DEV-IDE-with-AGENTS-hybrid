...
**Date:** 2026-06-19 · **Last updated:** 2026-06-30 (**RECONCILED forward** — the braid COMPLETED and three further arcs shipped since 2026-06-24; current work is **production-codebase hardening in service of the Marcus-SPOC product**, surfaced/prioritized by off-the-books proofing runs)
...
> **✅ 2026-06-30 — RECONCILED FORWARD (braid COMPLETE; three arcs shipped since; live frontier = the CONCIERGE PRODUCTION SUBSTRATE arc).** The 2026-06-24 "braid is the next frontier" framing below is now HISTORY — the braid shipped in full and three further arcs closed. Sequence since:
...
**Last pathway update:** 2026-06-30 (braid COMPLETE incl. S5 conversational SPOC; P5 consumption arc + directed-voice + enhanced-VO arcs shipped; live frontier advanced to the **Concierge Production Substrate arc** — Leg-1a done, the coverage-assurance interlock [Leg-1] DUAL-GATE CLOSED path A, Leg-1b next).

### 11.1 You are here

**(2026-06-30 — CURRENT) The GOAL is the Marcus-SPOC product (the runtime orchestrator driving a real APP instance); its outputs — the narrated-deck, the cited WORKBOOK companion, and the honest conversational Marcus — are ALL LIVE. The current work is HARDENING the production codebase so the SPOC-orchestrated runtime is trustworthy (fidelity-firewalled) — work that off-the-books concierge/proofing runs helped surface and prioritize, in service of the product (per the framing principle at the top: the concierge run is the discovery vehicle, never the design target).** Four arcs closed since the braid green-light below:
...
[CONCIERGE PRODUCTION SUBSTRATE ARC — CURRENT (opened 2026-06-29); true-production deck+motion+workbook+Descript via 4 legs]
  ...
  Leg-1b warm_callback authoring + Vera-R7 teeth ... ▶ NEXT (DUAL-GATE; own party green-light; consumes Leg-1 source_point anchors)
  Leg-2 motion bundle composition (B2/B3 live) .... ❌ queued
  Leg-3 callback + intelligent clustering ONLINE .. ❌ queued (read-only confirm-spike first)
  Leg-4 asset/fidelity ledgers .................... ❌ queued — INHERITS: narration-figure-fidelity enforcement · figure_tokens neck under-extraction · non-figure-detail coverage semantics · changed-numeral zero-tolerance · deferred real-run covered-PASS confirmation
  ...
  Terminal DoD: a true-production concierge run on prepared substrate ❌ after Legs 1b/2/3/4
...
| ▶ Concierge Leg-1b (warm_callback + Vera-R7 teeth) | 🔄 **NEXT BUILD** | DUAL-GATE; own party green-light; consumes Leg-1 source_point anchors |
| Concierge Legs 2/3/4 (motion / clustering / ledgers) | ❌ queued | Leg-4 inherits the narration-figure-fidelity + neck + semantic-coverage work |
| P2 epic close | ❌ before scale-up | fresh-naive-holdout still owed for generalization |
| BETA Phase 2 (T6 / T8 / full §8) | ◐ variant+voice+SPOC live; full §8 dress rehearsal open | after the concierge substrate hardens |

**You are here →** the **goal is the Marcus-SPOC product** (the runtime orchestrator driving a real APP instance); its outputs — the narrated-deck, the **cited WORKBOOK companion**, and the **honest conversational Marcus** — are all **end-to-end live** and individually proven (the braid shipped in full 2026-06-25; the P5 enrichment loop closed; directed voice + enhanced-VO landed). The reading-path classifier work was **superseded by the LLM-first ship** (numbers on the consumed-14; **fresh naive holdout** still the binding generalization gate). The current work is **hardening the production codebase** so the SPOC-orchestrated runtime is trustworthy and fidelity-firewalled — production fixes that concierge/proofing runs **surfaced + prioritized in service of the product** (not codebase changes made to support those runs; see the framing principle at the top): Leg-1a (rhetorical_role) done; the coverage-assurance interlock (Leg-1) **DUAL-GATE CLOSED path A** (numeral/figure-token facet; narration-figure-fidelity routed to Leg-4); **Leg-1b (warm_callback) is the next build**, then Legs 2/3/4. A full Concierge **Part-1 narrated lesson** was already published to Descript in parallel — an off-the-books proofing run that demonstrates the assembled pipeline already delivers.
...
**Horizon 2 — BETA spec as written (`beta-spec §8`).** **Now partly live:** genuine per-slide variant picks + presenter voice are demonstrated error-free twice to Descript (2026-06-24); voice candidate→bind proven; next-trial party-certified. **Open:** the **real conversational Marcus SPOC** (LLM stop-and-chat REPL, beyond the scripted approve-path) — the live frontier; Tracy on the trial path (T6d); dedicated lesson-plan / ingestion review surfaces (T6b/c); motion carve-out ratified or an honest producer (T8); a full §8 dress rehearsal exercising the **conversational a+b+c path** (variant/voice picks already covered). **Exit:** the conversational Marcus product on the frozen corpus, error-free twice **to spec**.
...
| Concierge Production Substrate arc (4 legs) | **~30% (Leg-1a + Leg-1 done)** | Leg-1a rhetorical_role emission DONE; coverage-assurance interlock (Leg-1) DUAL-GATE CLOSED path A (BLOCK arm LIVE, COVERED arm OFFLINE; numeral-facet only). Leg-1b NEXT; Legs 2/3/4 queued; Part-1 narrated lesson PUBLISHED to Descript in parallel |
| BETA Phase 2 / full §8 spec | **~55–65%** | variant+voice picks + the real conversational SPOC now LIVE; open: motion (T8), Tracy trial-path review surfaces (T6), full §8 dress rehearsal, the concierge true-production run |
| Full APP (Epics 15–18 + LP integration) | **~15–20%** | substrate exists; not yet wired to the operator product |

**Bottom line:** the three product pillars — the **narrated-deck**, the **cited WORKBOOK companion**, and the **honest conversational Marcus SPOC** — are all **end-to-end live and individually proven**. Since the braid green-light, four arcs closed: the **braid COMPLETE** (S5 SPOC `e20aadc`, research live-leg closed), the **P5 enrichment loop CLOSED** (workbook consumes the enriched corpus, byte-verified), the **directed synthesized-voice arc** shipped (directed ElevenLabs + acoustic verification + live Descript), and the **enhanced-VO arc** landed clip-level (v3 tag channel = a real-but-subtle control surface). **The goal is the Marcus-SPOC product (the runtime orchestrator driving a real APP instance); the current work is hardening the production codebase so that SPOC-orchestrated runtime is trustworthy and fidelity-firewalled** — production fixes that off-the-books concierge/proofing runs **surfaced and prioritized in service of the product, never codebase changes made to support the proofing runs themselves** (see the framing principle at the top). Under the "Concierge Production Substrate" arc: Leg-1a (real `rhetorical_role` emission) is done; the operator-inserted **coverage-assurance interlock (Leg-1)** — a fail-loud-before-spend fidelity firewall over every source-note point, a genuine production-runtime safety property — is **DUAL-GATE CLOSED (path A)** with an honest scope (numeral/figure-token facet only; the BLOCK arm is live-proven at $0 ElevenLabs, the COVERED arm offline-proven). The central honest finding it surfaced — **production narration is NOT yet figure-faithful** (it substitutes source figures, and the gate correctly blocks that) — is routed to **Leg-4** with named owners. **Next: Leg-1b (warm_callback) → Leg-2 (motion) → Leg-3 (clustering) → Leg-4 (fidelity ledgers).** A full Concierge **Part-1 narrated lesson** was already published to Descript (2026-06-30, every gate honored, zero code edits) — an off-the-books proofing run that demonstrates the assembled pipeline delivers an end-to-end lesson today (and is a source of production-defect findings, not a deliverable in itself). (The NEW CYCLE / fully-spawned-party T11 gate continues to earn its keep — this session it caught and prevented an offline-only over-claim, drove an honest live option-C attempt, and produced a dual-gate close that names exactly what is and isn't proven.)
