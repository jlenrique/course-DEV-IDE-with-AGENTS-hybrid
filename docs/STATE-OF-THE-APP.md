...
**Date:** 2026-06-19 · **Last updated:** 2026-06-24 PM-2 (**this doc RECONCILED** to reality + **next frontier RATIFIED** — clustering terminal-(b) + Descript-publish + fidelity L1/L2 absorbed; stale "live frontier = conversational SPOC" REPLACED by the party-green-lit **braid**: Marcus-interlocutor + research foundations + lesson-planning-with-workbook-companion)
...
**Last pathway update:** 2026-06-24 PM-2 (clustering arc CLOSED at terminal-(b) + Descript video PUBLISHED + fidelity L1/L2 landed; **braid green-light 6/6** ratified the next frontier — workbook-companion-first; the conversational SPOC is demoted to arc finale S5. You-are-here advanced to: the braid, Slice 1 specs → dev).

### 11.1 You are here

**(2026-06-24 PM-2 — CURRENT) The narrated-deck product is END-TO-END LIVE and PUBLISHED; the live frontier is now the party-ratified BRAID, sequenced client-value-first.** Since the SPOC-framed snapshot below, two arcs closed and one opened:
...
[THE BRAID — RATIFIED NEXT FRONTIER (green-light 6/6, 2026-06-24 PM-2); client-value-first]
  ...
  Live small-scale validation (frozen tejal) ...... ◐ Leg A workbook PROVEN-LIVE (real .docx + real word/media figure) · Leg C overlay --check exit 0 · Leg B research wiring trial-path-verified but live Scite/Consensus BLOCKED on operator account tier (HTTP 401 premium + CONSENSUS_API_KEY unset)
  S5 ▶ REAL conversational Marcus SPOC (LLM REPL) .. ❌ ARC FINALE — stop-and-chat; reads capability-state as fact; replaces scripted narrator. Class S; party → dev → T11
...
| ▶ Braid Slice 1 (workbook companion) | 🔄 **NEXT BUILD** | S1 lesson-plan spec → S2 producer (MD→DOCX) → S3 research wire; ships the client's cited workbook |
| Braid Slice 2 / REAL conversational Marcus SPOC (S5) | ❌ ARC FINALE | LLM stop-and-chat REPL on a GENERATED capability-self-model; replaces scripted narrator. Class S |
| P2 epic close | ❌ after the SPOC build / before scale-up | fresh-naive-holdout still owed for generalization |
| BETA Phase 2 (T6 / T8 / full §8) | ◐ variant+voice live; conversational path open | after the real SPOC |

**You are here →** the narrated-deck product is **end-to-end live** (LLM-first perception, presenter voice, genuine per-slide A/B variant selection, error-free twice to Descript) and the **next trial is party-certified**. The reading-path classifier work (P2-4c S1/S2/S3) was **superseded by the LLM-first ship**; its accuracy numbers stand on the consumed-14 (a **fresh naive holdout** remains the binding generalization gate). Clustering then CLOSED at terminal-(b) (PUBLISHED as a Descript video). The next milestone is the **braid — the cited WORKBOOK companion first** (Slice 1: lesson-plan collateral spec → workbook producer [MD→DOCX] → thin research wiring), then the honest conversational Marcus SPOC as the **arc finale** (Slice 2, on a generated capability-self-model) — not more classifier calibration.
...
| Braid (workbook + research + interlocutor) | **~65% (Slice 1 + S4 DONE)** | S1/S2/S3/S4 + Story 0/0b all committed through full gate (S2/S3 survived T11 HAND-BACK remediations); workbook PROVEN-LIVE on frozen tejal; research wiring trial-path-verified (live retrieval operator-gated on account tier). Remaining: S5 conversational REPL + the live external research leg |
| BETA Phase 2 / full §8 spec | **~45–55%** | variant+voice picks now LIVE + presenter voice; **real conversational SPOC** (the gating piece), research/motion/full-conversational-path open |
| Full APP (Epics 15–18 + LP integration) | **~15–20%** | substrate exists; not wired to the operator product |

**Bottom line:** the narrated-deck product is **end-to-end live and the next trial is party-certified** — LLM-first perception, presenter-voice narration, and genuine **per-slide A/B variant selection** all completed **error-free twice to Descript** (2026-06-24, real Playwright chooser clicks). The reading-path classifier sub-arc (P2-4c S1/S2/S3) was **superseded by the LLM-first ship**; its numbers stand on the consumed-14, so a **fresh naive holdout** remains the binding generalization gate. The clustering / VO-follow-along arc CLOSED at terminal-(b) (a clustered per-sub-slide A/B run PUBLISHED as a Descript video; fidelity L1/L2 landed). **The live frontier is now the party-ratified BRAID — Marcus-interlocutor + research foundations + lesson-planning-with-workbook-companion — sequenced client-value-first: ship the durable, cited WORKBOOK companion (Slice 1) before any Marcus net-new (Slice 2); the conversational SPOC is the arc FINALE (S5), built on a GENERATED capability-self-model so it cannot promise unbuilt capability.** After the braid: P2 epic close → fresh-naive-holdout → **BETA charter remainder (T6/T8 + full §8)** → **aspirational epics (15–18)**. (The NEW CYCLE T11 gate has earned its keep repeatedly — over-claim, vacuous-gate, index-misalignment, and believed-green attribution errors caught before they shipped; the braid's biggest believed-green risk is Marcus's capability-self-model, gated by the generated overlay + adversarial over-promise probe.)
