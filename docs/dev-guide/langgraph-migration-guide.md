...
## Cost Engineering Foundation (Slab 5 Story 5a.3)

Story 5a.3 adds a migrated-runtime cost-accounting layer rather than pretending
the legacy branch had a comparable token baseline. `app/runtime/economics.py`
reads LangSmith trace spans, attributes token usage per agent and per model,
applies the operator-maintained pricing table in
`runtime/config/openai_pricing.yaml`, and persists strict per-trial reports to
`state/config/runs/<trial-id>/cost-report.{json,md}`.

The model cascade is intentionally parameterized in
...
Two guardrails ship with the report surface:
