...
| 2026-04-29 | Class-C+ canonical sanctum pattern is four-file BMB (`INDEX.md` / `PERSONA.md` / `chronology.md` / `access-boundaries.md`), distinct from Class-A six-file BMB. Tracy is the seed exemplar; future Class-C+ specialists inherit this pattern. Related artifacts: `scripts/utilities/validate_parity_test_class_conformance.py` Class-C+ template and `docs/dev-guide/sanctum-reference-conventions.md`. | Class-C+ port-shape work is lighter than Class-A persona hardening; four files capture activation context, persona, chronology, and access boundaries without the full CREED/BOND/MEMORY/CAPABILITIES set. | Slab 7b Story 7b.5 Tracy port-shape + sidecar creation | Codex implementation per NFR-CG17 + Story 7b.5 AC-M |
| 2026-04-29 | Class-C canonical sanctum pattern is six-file BMB (`INDEX.md` / `PERSONA.md` / `CREED.md` / `BOND.md` / `MEMORY.md` / `CAPABILITIES.md`), matching Class-A and distinct from Class-C+ four-file sidecar. Gary is the seed exemplar; 7b.7 Kira, 7b.8 Enrique, and 7b.9 Wanda inherit this Class-C pattern. | API-bound Class-C specialists still participate in operator-facing G2/G3 review loops and need full persona continuity around API outputs, artifact handoff, budget discipline, and downstream fidelity hooks. | Slab 7b Story 7b.6 Gary Class-C port-shape | Codex implementation per NFR-CG17 + Story 7b.6 AC-O |
| 2026-04-30 | Class-D2 sidecar variant is canonical per D-Class-2-codification (renamed from "D20" at S6 2026-05-08 per Winston disambiguation; the original "D20" tag collided with the architecture doc's reserved D20 evidence-slot; class codifications now use a distinct non-numeric tag): four-file operational-metadata sidecar at `_bmad/memory/bmad-agent-compositor/` (`contract.md` / `version.md` / `chronology.md` / `access-boundaries.md`), SKILL.md at `skills/compositor/SKILL.md`, no `skills/bmad-agent-compositor/`, and exemption from FR101.iv sanctum-path-equality. Compositor is the seed exemplar for Class-D2; future D2 specialists inherit this pattern. Related artifacts: `scripts/utilities/validate_parity_test_class_conformance.py` Class-D2 template and `docs/dev-guide/scaffolds/scaffold-v0.2-D2-pipeline/`. | D2 pipeline-greenfield specialists are deterministic assembly components with no LLM and no third-party API. Operational metadata matches their role; persona-continuity files are structurally inapplicable. | Slab 7b Story 7b.11 Compositor greenfield + FR111 scaffold-v0.2-D2-pipeline first consumption | Codex implementation + Story 7b.11 AC-K |
| 2026-05-05 | §02A LLM-driven directive composer body landed at `app/composers/section_02a/composer.py`; Trial-2 finding #2 STRUCTURALLY retired. Composer reads corpus dir contents, constructs LLM prompt for semantic role assignment per file, discriminates by file format (skip `.gitkeep`; PNG/JPG/PPTX/PDF as visual references with no `expected_min_words`; identify primary `.docx`/`.md`/`.pdf` by content sampling), produces operator-reviewable directive YAML at G0. Forensic regression test asserts non-byte-identicality with broken corpus-scan fixture + invariants on `.gitkeep` IGNORED + `.docx` PRIMARY + binary `expected_min_words=None`. | Trial-2 attempts 1, 5, 6, 7, 8 produced byte-identical broken corpus-scan directive (5 successive runs); pre-Slab-7c §02A composer was a fallback enumeration of corpus-dir files, NOT LLM-driven semantic role assignment. The §02A LLM body is the SUBSTRATE-level fix; composer pivot is the disposition. | Slab 7c Story 7c.3a close + cross-agent MANDATORY review PASS | Operator + cross-agent review |
| 2026-05-07 | W5 Marcus-writer partition principle codified at architecture-of-record (D19): Marcus is sole envelope-contribution author; specialists are pure functions; runner is sole graph-execution authority. Three-way partition structurally enforced via M1 single-writer rule + D3 HIL tamper-evidence + C6 §section independence. | Slab 7c shipped 14 §section packages each with `poll_surface.py`; W5 amendment to slab-7c PRD codified the partition; without architecture-tier codification, future §section authors would be tempted to duplicate write surfaces. | Pre-Trial-3 cleanup S5 Tier-2 + Slab 7c PRD W5 amendment | Operator + 3-agent pre-S5 party-mode (Mary lead + Murat + Winston) |
| 2026-05-07 | Eight-family gate taxonomy LOCK (G0/G1/G2/G2A/G2C/G3/G4/G5/G6) per ADR 0002. Promoted to architecture-of-record D18. Alias gates inherit family semantics via `alias_of` clause; runtime gate IDs (currently 18) validated against eight-family taxonomy at compile time; `production_gate_ids(manifest)` derives runner-pause set from manifest fold-flag metadata (currently {G1, G2C, G3, G4}; 4 of 18). | Slab 7c authored 14 §section packages each declaring a gate code; the C6 `independence` import-linter contract enumerates the §section list; without the eight-family taxonomy, §section package layout would be ad-hoc. | Pre-Trial-3 cleanup S5 Tier-2 (Winston D18 recommendation) | Operator + ADR 0002 ratification + Winston pre-S5 review |
