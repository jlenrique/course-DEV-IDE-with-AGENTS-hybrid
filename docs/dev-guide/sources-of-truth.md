...
| Marcus namespace home | `app.marcus.*` (single canonical package) | `app.marcus.intake/` + `app.marcus.orchestrator/` + `app.marcus.facade.py` + `app.marcus.lesson_plan/` + `app.marcus.dispatch/` | D14 architecture-of-record entry; M5 import-linter contract bans top-level `marcus.*` imports. Story 30-1 INTERNAL duality (intake ↔ orchestrator) preserved per R1 amendments 12+13+17 (Murat Golden-Trace + Quinn single-writer + Maya-as-one-voice). Legacy `marcus/` retired at S2 close 2026-05-07 (commit `195be7c`). Future dual-namespace patterns require party-mode ratification. |
| Production prompt pack (canonical for production runs) | `docs/workflow/production-prompt-pack-v5-narrated-lesson-with-video-or-animation.md` | `app/marcus/cli/trial.py` (runtime entrypoint); `app/marcus/orchestrator/production_runner.py`; `skills/bmad-agent-marcus/references/specialist-registry.yaml`; `docs/dev-guide/adr/0002-slab-7c-gate-taxonomy.md`; `docs/trials/methodology.md` | v5 = canonical post-S4 (2026-05-07). v4.2 frozen as legacy-axis mapping authority for `slab-7-legacy-migrated-mapping-checklist.md`; v4.3 fully superseded-by-v5. Tier-1 (prose) = dev-agent; Tier-2 (new step) = party-mode; Tier-3 (new pack family) = full party-mode + operator sign-off. |
| Trial-run methodology (recurring process) | `docs/trials/methodology.md` | `docs/trials/cross-trial-learnings.md`; `docs/trials/trial-N-templates/{launch,log,postmortem}.md`; `docs/trials/trial-N/`; `_bmad-output/implementation-artifacts/trial-3-readiness-checklist.md` | S3 deliverable (2026-05-07). Tier-1 prose = operator authority; Tier-2 structural = party-mode (Quinn-R + Murat + Mary minimum); Tier-3 governance = full party-mode + operator sign-off. Refresh trigger: every 3 trials OR Epic-close retrospective. |

### Legacy production prompt-pack authority

| Topic | SSOT | Lockstep partners | Change protocol |
|---|---|---|---|
| Legacy v4.2 motion-enabled prompt-pack contract | `docs/workflow/production-prompt-pack-v4.2-narrated-lesson-with-video-or-animation.md` | `_bmad-output/planning-artifacts/slab-7-legacy-migrated-mapping-checklist.md` (mapping rows); `state/config/pipeline-manifest.yaml` (33-node v4.2 manifest); `app/manifest/compiler.py::production_gate_ids(manifest)` (manifest-derived) | **FROZEN at S4 close 2026-05-07** for mapping-checklist legacy-axis purposes. NO further edits permitted. v5 is canonical for production runs. |
| Legacy v4.1 non-motion prompt-pack contract | `docs/workflow/production-prompt-pack-v4.1-narrated-deck-video-export.md` | sibling of v4.2; non-motion runs only | Same status as v4.2 (legacy-frozen for mapping axis). |
| Legacy v4.3 cluster + interstitial prompt-pack | `docs/workflow/production-prompt-pack-v4.3-narrated-lesson-with-video-or-animation.md` | superseded-by-v5 at S4 close 2026-05-07 (commit `a713112`) | Fully superseded; preserved on disk for git-history reference; DO NOT EDIT. Cluster-mode content already absorbed into v4.2 §05B/§6.2/§6.3/§7.5 placeholders. |
| Legacy mapping checklist | `_bmad-output/planning-artifacts/slab-7-legacy-migrated-mapping-checklist.md` | v4.2 prompt-pack section structure; FR-7c-42 row-flip authority | Party-mode-ratified row flips ONLY (per CLAUDE.md deferred-inventory governance); no dev-agent edits. |
| Operator Irene A/B tuning loop | `docs/workflow/operator-script-v4.2-irene-ab-loop.md` | inserts between v4.2 §05/§05B and §06; references Epic 23 cluster mode | Active per its own `status: active` front-matter; v5 will absorb at next major version bump. |
| Production checkpoints (anti-drift) | `docs/workflow/human-in-the-loop.md` lines 31-43 | v4.2 §05 / §06B / §07C / §07D / §07F / §08B / §09 gate language | Lockstep with v4.2 prompt-pack body (FROZEN); cross-references to v5 as canonical. |
| Marcus internal duality (Intake / Orchestrator) | `_bmad-output/implementation-artifacts/30-1-marcus-duality-split.md` | `app.marcus.intake.INTAKE_MODULE_IDENTITY` ("marcus-intake"); `app.marcus.orchestrator.ORCHESTRATOR_MODULE_IDENTITY` ("marcus-orchestrator"); `NEGOTIATOR_SEAM` ("marcus-negotiator"); `app.marcus.facade.MARCUS_IDENTITY` ("marcus") + `MARCUS_DISPLAY_NAME` ("Marcus"); single-writer rule enforced at `emit_pre_packet_snapshot` writer-identity check; tests/test_marcus_facade_leak_detector.py + tests/contracts/test_no_intake_orchestrator_leak_marcus_duality.py | R1 amendments 12+13+17 binding; party-mode required to alter; programming-token strings are 31-2 WriterIdentity Literal values + Golden-Trace fixture content (PACKAGE-INDEPENDENT — relocate the package without touching the strings) |
...
| Topic | SSOT | Lockstep partners | Change protocol |
|---|---|---|---|
| Anti-pattern catalog | `docs/dev-guide/specialist-anti-patterns.md` (A1–A20 + P1–P4 post-S1 P0-4 dedup) | `docs/dev-guide/dev-agent-anti-patterns.md` (dev-cycle-level); per-specialist counter-pattern citations | Mary harvest-gate authority for new entries; format-freeze v1 preserved (existing entries don't get rewritten) |
