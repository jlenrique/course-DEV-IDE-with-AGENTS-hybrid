...
| Topic | SSOT | Lockstep partners | Change protocol |
|---|---|---|---|
| Production gates (G1/G2C/G3/G4) | `app/manifest/compiler.py::PRODUCTION_GATE_IDS` | `app/gates/resume_api.py`; `app/models/decision_cards.py`; `state/config/pipeline-manifest.yaml` (gate nodes); `tests/integration/marcus/test_production_runner_gate_pause_resume.py` | FR34 invariant; party-mode for gate-set changes |
| Decision card schema | `app/models/decision_cards.py` (G1Card, G2CCard, G3Card, G4Card; DecisionCardMeta) | per-card schemas + goldens + shape-pin tests; `tests/unit/models/decision_cards/` | Pydantic-v2 four-file-lockstep |
