...
9. **Self-resolving inputs from the run dir without threading `run_dir`** — a specialist that reads run-dir-relative artifacts (segment manifest, `run.json`, bundle) must be threaded the ACTUAL run dir via `_runner_payload_for_specialist` (`runs_root / trial_id` in the payload's `run_dir` override), NOT left to fall back to the shipped `RUNS_ROOT` default. Under a non-default `--runs-root` the shipped default points at an empty dir and every run-dir read fails loud (live trial `503e54c1` paused at 07W on `segment-manifest.missing`). Add the specialist to the payload-threading seam; regression floor at `tests/specialists/workbook_producer/test_run_dir_threading_503e54c1.py`.
10. **Needing `production_envelope` in `_act`** — the dispatch adapter NULLS `state.production_envelope` for EVERY specialist (per-component isolation invariant), so a specialist that reads it will see `None` in composition mode even though the authority exists. Re-load the persisted authority from `<run_dir>/run.json` (`ProductionTrialEnvelope.model_validate_json(...).production_envelope`, guarded: contained + non-symlink) rather than trusting the live field, OR arrange for the runner to thread it explicitly. `workbook_producer` was the sole envelope-reader and error-paused `authority-missing` at 07W until it loaded the persisted envelope from the run dir (both band nodes 07W.1-07W.4 persist `run.json` before 07W runs).
11. **Assuming a budget-skipped node fails silently (no longer true — Epic 41, 2026-07-17)** — a specialist that never dispatches because the run ran out of budget used to let the trial reach `completed` with the node's deliverable missing (the `bc747b51` false-green: a `max_specialist_calls=1` throttle starved the terminal band). That throttle is removed, `MARCUS_TRIAL_BUDGET_USD` is now an enforced stop, and a **silently-skipped specialist fails loud in BOTH walks** (start walk + continuation walk — the standing two-walk gotcha). Do not add your own "skip quietly if over budget" branch inside `_act`; let the runner's fail-loud guard own that. See the dev-guide Epic 41/42/43 seams.
