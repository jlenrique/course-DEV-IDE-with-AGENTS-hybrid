...
### A18. PowerShell `>` redirection emits UTF-8 BOM, breaks byte-for-byte schema-match assertions

- **Example:** Story 7c.5.G1 (extend-and-audit migration of G1 DecisionCard) at T3: Codex executed the canonical schema-regeneration command `.venv/Scripts/python.exe -c "from app.models.decision_cards.g1 import G1Card; import json; print(json.dumps(G1Card.model_json_schema(), indent=2, sort_keys=True))" > app/models/decision_cards/schema/g1.v1.schema.json` exactly as inherited from the G0/G2A/G5/G6 fresh-author canonical pattern (Slab 7c stories closed 2026-05-05 at commits `e2aa599` + `f059e84`). Windows PowerShell 5.1 (project default per `agent-environment.md` + `CLAUDE.md`) emits a UTF-8 BOM (`\xef\xbb\xbf` 3-byte prefix) at the start of files written via `>` redirection — even when the producing process emits plain UTF-8 on stdout. The G1 shape-pin's byte-for-byte schema-match assertion (`G1Card.model_json_schema()` regenerated via `json.dumps(...).encode("utf-8")` compared to the on-disk file) failed because the on-disk file carried the BOM prefix and the Python regeneration did not. Codex correctly diagnosed the encoding mismatch and patched by rewriting the file as UTF-8 without BOM via Python `Path.write_text(..., encoding="utf-8")`. Bug latent in G0/G2A/G5/G6 specs but did NOT manifest there because Codex's actual implementations on those used file-write helpers that bypassed `>` (lucky pattern divergence). G1 was the first time the bare `>`-redirection canonical command path was actually executed end-to-end on Windows.

- **Counter-pattern:** Specify schema-regeneration (and any deterministic-byte-output file emission) via Python `Path.write_text(..., encoding="utf-8")` (or equivalent `open(..., "w", encoding="utf-8", newline="")` + `json.dump(...)`) directly in the Codex prompt's T3 task — never via shell `>` redirection on Windows. The canonical replacement command:

  ```bash
  .venv/Scripts/python.exe -c "from pathlib import Path; from app.models.decision_cards.<gate> import <Gate>Card; import json; Path('app/models/decision_cards/schema/<gate>.v1.schema.json').write_text(json.dumps(<Gate>Card.model_json_schema(), indent=2, sort_keys=True), encoding='utf-8')"
  ```

  No `>` redirection; no BOM possible. Universal applicability across PowerShell / cmd.exe / bash on the same machine. Apply to ALL Codex prompts going forward (Slab 7c remaining: G2C / G3 / G4 extend-and-audit + downstream Wave-3+ schema-emission steps). Bonus discipline: any spec or Codex prompt that emits a file destined for byte-for-byte test assertion MUST use the Python file-write idiom in its canonical command, NOT shell redirection.

- **Slab-of-discovery:** Slab 7c story 7c.5.G1 (T3, 2026-05-05). Operator-flagged at the moment of Codex's BOM diagnosis + manual rewrite. **Slab-of-origin:** the canonical schema-regen command pattern shipped in the G0 fresh-author spec (Slab 7c story 7c.5.G0, 2026-05-05; spec at commit `e2aa599`) and propagated verbatim to G2A/G5/G6 fresh-author specs + G1 extend-and-audit prompt. The pattern was implicitly Linux-shell-portable but explicitly PowerShell-hostile. **Distinction from A11 (cp1252 NNBSP crash):** A11 was input-side encoding handling under `print()` at runtime; A18 is output-side encoding under shell redirection at file-write. Both are Windows-portability anti-patterns; treat as same family but distinct entries. The 7c.2 cp1252-fix story (closed earlier this session) was a related fix but did NOT cover the schema-regen redirection vector — incomplete-fix lesson per A11 corollary.
...
### A20. Cross-package helper duplication under import-isolation contracts

- **Example:** Slab 7c shipped 14 §section poll-surface modules (`app/gates/section_*/poll_surface.py`) each containing byte-identical local definitions of `canonical_model_bytes` + `compute_model_digest` (~30 LOC × 14 = ~420 LOC duplication). The duplication was forced by C6 import-linter contract `independence` between `app.gates.section_*` modules — no §section may import another §section. Re-emitting the helpers locally was the only way to honor C6 within the §section package boundary, but it produced a maintenance hazard: a future correctness fix to the helper must be applied 14 times in lockstep.
- **Counter-pattern:** Extract shared helpers to a sibling package that is **outside** the import-isolation contract's modules list. For Slab 7c: `app/gates/_common/digest_helpers.py` (NOT in C6 modules list — `_common` may be imported by any §section package). The helper file lives in the same parent namespace (`app.gates`) but escapes the `independence` contract by virtue of NOT being a `section_*` module. This preserves cross-§section isolation (no §section imports another §section) while eliminating the duplication. Pattern generalizes: any time `independence` or `forbidden` import-linter contracts force local re-emission across N siblings, extract the shared surface to a `_common` / `_shared` / `_lib` sibling package outside the contract's modules list.
- **Slab-of-discovery:** Slab 7c Story 7c.7 T11 review (filed as `digest-helpers-extract-to-app-gates-_common` follow-on; carried forward across all 14 §section closes; remediation pre-authored at session-WRAPUP 2026-05-07 as `migration-7c-housekeeping-1-digest-helpers-extract`).
...
### P1. Facilitator-skill referenced as dev-agent action in epic-level AC

- **Example:** Story 7c.21's epic-level AC text (line 935 of `epics-slab-7c-orchestrational-tail.md`) read *"the dev-agent triggers `bmad-retrospective`"*. This is a structural mismatch: `bmad-retrospective` is a facilitator-driven multi-agent workflow that requires operator + party-mode participation; it is NOT executable under `bmad-dev-story` discipline. A dev-agent attempting to "trigger bmad-retrospective" would either (a) confuse `bmad-dev-story` with skill-invocation discipline, (b) silently no-op, or (c) produce a placeholder retrospective artifact that doesn't actually capture multi-agent ratification. The defect was caught at pre-author time during 7c.21 spec authoring (session-WRAPUP 2026-05-06) and carved out: dev-agent scope = retrospective EVIDENCE PREPARATION (markdown pack); operator-driven Gate-2 = SKILL TRIGGER.
- **Counter-pattern:** When an epic AC mentions a multi-agent facilitator skill (`bmad-retrospective`, `bmad-party-mode`, `bmad-create-prd`, `bmad-create-epics-and-stories`, `bmad-correct-course`, etc.), the dev-agent's scope is the EVIDENCE PREPARATION feeding the skill, NOT the skill TRIGGER. Spec authors should explicitly carve out: dev-agent prepares (markdown evidence pack / structured fixtures / pre-populated state); operator-driven Gate-2 (or party-mode round / retrospective ceremony) actually triggers the facilitator skill consuming that preparation. Document the carve-out at the top of the spec to internalize the boundary; verify in code-review that the dev-agent diff does NOT attempt direct skill invocation.
- **Slab-of-discovery:** Slab 7c Story 7c.21 spec pre-author (Claude Opus 4.7 session-WRAPUP 2026-05-06). The corrected carve-out shipped in the 7c.21 spec body §"Scope carve-out: dev-agent vs operator-driven Gate-2"; Codex internalized it cleanly (the retrospective evidence pack opens with explicit non-trigger disclaimer).

### P2. DISMISS-thread as latent-anti-pattern signal

- **Example:** Across Slab 7c Wave-3 and Wave-5 reviews (verdicts 7c.9, 7c.10, 7c.11, 7c.12, 7c.13, 7c.14, 7c.20b — seven independent T11 reviews), the inherited `test_no_unauthorized_callers` failure was DISMISSED as PRE-EXISTING noise each time. Each verdict correctly noted the failure was inherited (not introduced by the story under review) but did NOT escalate the meta-observation that "seven independent reviewers all dismiss the same finding" is itself a signal that something is structurally wrong. The actual diagnosis (substring scanner catches class definitions; filename exclusion glob too narrow) only landed at 7c.21 close as anti-pattern A19 — after seven dismissal cycles.
- **Counter-pattern:** When a code-review verdict DISMISSES an inherited failure, attach a "DISMISS-thread counter" to the failure-id. When the counter reaches 3-5, escalate the meta-observation: file as candidate substrate or test anti-pattern + propose remediation story. The DISMISS pattern is appropriate for genuine inherited noise (e.g., xdist ordering; fixture state; external API flake), but a ratchet-to-anti-pattern at threshold N prevents long-term DISMISS-thread accumulation. Operationalize via review-checklist line at T11 standard tier: "if this finding is DISMISSED-as-inherited, has it been DISMISSED ≥3 times across prior verdicts? If yes, escalate to anti-pattern harvest."
- **Slab-of-discovery:** Slab 7c retrospective-evidence-pack authoring 2026-05-06; cross-verdict pattern observed across 7c.9/10/11/12/13/14/20b dismissal cycles for `test_no_unauthorized_callers`. Remediation A19 anti-pattern + housekeeping story `migration-7c-housekeeping-2-scanner-staleness-rewrite` filed at session-WRAPUP 2026-05-07.

### P3. Two-agent NEW CYCLE requires operator-as-bridge

- **Example:** Slab 7c marathon NEW CYCLE (Claude pre-authors specs + reviews verdicts; Codex T1-T10 dev) ran 36 stories end-to-end PASS-zero-patches × 36, but required operator-frequent-intervention as the CONNECTING TISSUE between agents. Every spec needed operator dispatch to Codex; every Codex T10 dropbox needed operator notification to Claude; every close-batch needed operator commit-ratification. Operator surfaced this at Slab 7c retrospective: *"Not being able to have Claude Code and Codex Agent coordinate their efforts without my frequent intervention as 'bridge'."* The friction is real and structural — not a Slab-7c-specific defect but a NEW CYCLE design property.
...
- **Slab-of-discovery:** Epic 34 (post-Slab-7c Section 02A downstream-consumer coherence; Murat M-Murat-2 binding from Phase B SCP-ratification Round 1 2026-05-22)
