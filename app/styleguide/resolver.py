...
Canonical-arc S1 (D1, 2026-07-06): re-homed VERBATIM from
``app/specialists/gary/styleguide_library.py`` to this NEUTRAL module so the
resolution core is importable from ``app.specialists.cd``,
``app.specialists.gary``, and ``app.marcus.orchestrator`` without any of them
importing ``app.specialists.gary`` (strangler-fig; the gary module is now a
thin byte-compatible re-export). Pure functions; no run-state; no I/O beyond
the yaml read.

This module is the shared spine for the Gary consumption seam
(``_normalized_gamma_settings``), CD's ``styleguide_resolution`` audit
emission (canonical-arc S1), and the offline write-gate
...
def load_style_guides(
    ...
    *,
    ...
    Every load/parse failure (missing file, non-UTF-8 bytes, malformed YAML,
    non-mapping top-level, empty ``style_guides``) is wrapped in
    ``StyleguideError`` (tag ``gamma.styleguide.load-error``) so Gary's
    ``except StyleguideError`` path error-pauses recoverably instead of
    crashing the walk with a bare ``FileNotFoundError`` / ``yaml.YAMLError`` /
    ``ValueError`` (code-review item #2).

    ``content`` (S1 remediation T5, additive + optional): when supplied, the
    caller has ALREADY read the SSOT bytes (typically to digest them) and this
    function parses exactly those bytes/text instead of re-reading ``path`` —
    closing the TOCTOU gap where a digest attests different bytes than the
    resolution parsed. ``path`` is still used for error-message context.
    Default behavior (``content=None``) is byte-identical to before.
    """
