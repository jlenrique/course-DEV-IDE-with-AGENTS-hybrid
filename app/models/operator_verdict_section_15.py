"""Section 15 HIL G5 final-operator-handoff verdict models."""
...
Section15VerdictVerb = Literal["complete", "edit", "reject"]
Section15SurfaceId = Literal["section_15_g5_final_handoff"]
SECTION_15_SURFACE_ID: Section15SurfaceId = "section_15_g5_final_handoff"
...
class HandoffEditPayload(BaseModel):
    """Field-level edits for the reviewed final handoff package."""
    ...
    edits: dict[str, Any] = Field(
        ...,
        min_length=1,
        description="Mapping of final-handoff field names or paths to replacement values.",
    )
...
class Section15OperatorVerdict(BaseModel):
    """Tamper-evident verdict emitted by the Section 15 G5 poll surface."""
    ...
    run_id: UUID = Field(
        ...,
        description="UUID4 identity for the final-operator-handoff run.",
    )
    surface_id: Section15SurfaceId = Field(
        default=SECTION_15_SURFACE_ID,
        description="Closed Section 15 final-handoff surface identifier.",
    )
    verb: Section15VerdictVerb = Field(
        ...,
        description="Closed verdict verb set: complete | edit | reject.",
    )
    handoff_id: str = Field(
        ...,
        min_length=1,
        description="Identifier of the G5 final handoff under review.",
    )
    ...
    decision_card_digest: str = Field(
        ...,
        description="Lowercase sha256 digest of the reviewed G5 card.",
    )
    edit_payload: HandoffEditPayload | None = Field(
        default=None,
    ...
    @field_validator("handoff_id", "operator_id", mode="before")
    @classmethod
    ...
    def _enforce_verb_payload_consistency(self) -> Section15OperatorVerdict:
        if self.verb == "edit":
        ...
        if self.edit_payload is not None or self.reject_reason is not None:
            raise ValueError("verb='complete' forbids edit_payload and reject_reason")
        return self
