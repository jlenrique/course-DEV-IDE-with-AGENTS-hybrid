"""Section 08B HIL Storyboard B verdict models."""
...
Section08BVerdictVerb = Literal["approve", "edit", "reject"]
Section08BSurfaceId = Literal["section_08b_g3b_poll"]
SECTION_08B_SURFACE_ID: Section08BSurfaceId = "section_08b_g3b_poll"
...
class StoryboardBEditPayload(BaseModel):
    """Field-level edits for the reviewed Storyboard B payload."""
    ...
    edits: dict[str, Any] = Field(
        ...,
        min_length=1,
        description="Mapping of Storyboard B field names or paths to replacement values.",
    )
...
class Section08BOperatorVerdict(BaseModel):
    """Tamper-evident verdict emitted by the Section 08B G3B poll surface."""
    ...
    run_id: UUID = Field(
        ...,
        description="UUID4 identity for the Storyboard B review run.",
    )
    surface_id: Section08BSurfaceId = Field(
        default=SECTION_08B_SURFACE_ID,
        description="Closed Section 08B poll-surface identifier.",
    )
    verb: Section08BVerdictVerb = Field(
        ...,
    ...
    storyboard_id: str = Field(
        ...,
        min_length=1,
        description="Identifier of the Storyboard B card under review.",
    )
    ...
    decision_card_digest: str = Field(
        ...,
        description="Lowercase sha256 digest of the reviewed G3 card.",
    )
    edit_payload: StoryboardBEditPayload | None = Field(
        default=None,
    ...
    @field_validator("storyboard_id", "operator_id", mode="before")
    @classmethod
    ...
    def _enforce_verb_payload_consistency(self) -> Section08BOperatorVerdict:
        if self.verb == "edit":
