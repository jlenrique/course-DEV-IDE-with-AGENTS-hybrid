"""Section 11B HIL G4B input-package verdict models."""
...
Section11BVerdictVerb = Literal["approve", "edit", "reject"]
Section11BSurfaceId = Literal["section_11b_g4b_input_package"]
SECTION_11B_SURFACE_ID: Section11BSurfaceId = "section_11b_g4b_input_package"
...
class InputPackageEditPayload(BaseModel):
    """Field-level edits for the reviewed G4B input package."""
    ...
    edits: dict[str, Any] = Field(
        ...,
        min_length=1,
        description="Mapping of input-package field names or paths to replacement values.",
    )
...
class Section11BOperatorVerdict(BaseModel):
    """Tamper-evident verdict emitted by the Section 11B G4B poll surface."""
    ...
    run_id: UUID = Field(
        ...,
        description="UUID4 identity for the input-package preview run.",
    )
    surface_id: Section11BSurfaceId = Field(
        default=SECTION_11B_SURFACE_ID,
        description="Closed Section 11B input-package surface identifier.",
    )
    verb: Section11BVerdictVerb = Field(
        ...,
        description="Closed verdict verb set: approve | edit | reject.",
    )
    input_package_id: str = Field(
        ...,
        min_length=1,
        description="Identifier of the G4B input package under review.",
    )
    ...
    edit_payload: InputPackageEditPayload | None = Field(
        default=None,
    ...
    @field_validator("input_package_id", "operator_id", mode="before")
    @classmethod
    ...
    def _enforce_verb_payload_consistency(self) -> Section11BOperatorVerdict:
        if self.verb == "edit":
