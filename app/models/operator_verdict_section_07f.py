"""Section 07F HIL motion gate verdict models."""
...
Section07FVerdictVerb = Literal["approve", "edit", "reject"]
Section07FSurfaceId = Literal["section_07f_g2f_motion_gate"]
SECTION_07F_SURFACE_ID: Section07FSurfaceId = "section_07f_g2f_motion_gate"
...
class MotionClipEditPayload(BaseModel):
    """Field-level edits for the reviewed motion-clip payload."""
    ...
    edits: dict[str, Any] = Field(
        ...,
        min_length=1,
        description="Mapping of motion-clip field names or paths to replacement values.",
    )
...
class Section07FOperatorVerdict(BaseModel):
    """Tamper-evident verdict emitted by the Section 07F G2F poll surface."""
    ...
    run_id: UUID = Field(
        ...,
        description="UUID4 identity for the motion-clip review run.",
    )
    surface_id: Section07FSurfaceId = Field(
        default=SECTION_07F_SURFACE_ID,
        description="Closed Section 07F poll-surface identifier.",
    )
    verb: Section07FVerdictVerb = Field(
        ...,
        description="Closed verdict verb set: approve | edit | reject.",
    )
    motion_clip_id: str = Field(
        ...,
        min_length=1,
        description="Identifier of the motion clip under review.",
    )
    ...
    edit_payload: MotionClipEditPayload | None = Field(
        default=None,
    ...
    @field_validator("motion_clip_id", "operator_id", mode="before")
    @classmethod
    ...
    def _enforce_verb_payload_consistency(self) -> Section07FOperatorVerdict:
        if self.verb == "edit":
