"""Section 07D HIL motion-plan polling verdict models."""
...
Section07DVerdictVerb = Literal["approve", "edit", "reject"]
Section07DSurfaceId = Literal["section_07d_g2_5_motion_plan_polling"]
SECTION_07D_SURFACE_ID: Section07DSurfaceId = "section_07d_g2_5_motion_plan_polling"
...
class MotionPlanEditPayload(BaseModel):
    """Field-level edits for the reviewed motion-plan payload."""
    ...
    edits: dict[str, Any] = Field(
        ...,
        min_length=1,
        description="Mapping of motion-plan field names or paths to replacement values.",
    )
...
class Section07DOperatorVerdict(BaseModel):
    """Tamper-evident verdict emitted by the Section 07D G2.5 poll surface."""
    ...
    run_id: UUID = Field(
        ...,
        description="UUID4 identity for the motion-plan polling review run.",
    )
    surface_id: Section07DSurfaceId = Field(
        default=SECTION_07D_SURFACE_ID,
        description="Closed Section 07D poll-surface identifier.",
    )
    verb: Section07DVerdictVerb = Field(
        ...,
        description="Closed verdict verb set: approve | edit | reject.",
    )
    motion_plan_id: str = Field(
        ...,
        min_length=1,
        description="Identifier of the completed motion-plan card under review.",
    )
    ...
    edit_payload: MotionPlanEditPayload | None = Field(
        default=None,
    ...
    @field_validator("motion_plan_id", "operator_id", mode="before")
    @classmethod
    ...
    def _enforce_verb_payload_consistency(self) -> Section07DOperatorVerdict:
        if self.verb == "edit":
