"""Generated 9-node scaffold graph for specialist tamara."""
...
class OperatorInstructionsParseError(RuntimeError):  # noqa: N818
    """Raised when Tamara operator-instructions envelope cannot be parsed."""
...
def _decode_envelope_payload(state: RunState) -> dict[str, Any]:
    if state.cache_state is None or not state.cache_state.cache_prefix:
        raise OperatorInstructionsParseError(
            "tamara act envelope cache_prefix is missing",
            tag="design_spec.parsed.missing-key",
        )
    try:
        decoded = json.loads(state.cache_state.cache_prefix)
    except json.JSONDecodeError as exc:
        raise OperatorInstructionsParseError(
            f"tamara act envelope cache_prefix is not valid JSON: {exc}",
            tag="design_spec.parsed.malformed",
        ) from exc
    if not isinstance(decoded, dict):
        raise OperatorInstructionsParseError(
            "tamara act envelope cache_prefix must decode to a mapping",
            tag="design_spec.parsed.wrong-type",
        )
    return decoded
...
def _parse_operator_instructions_envelope(payload: dict[str, Any]) -> dict[str, Any]:
    if "prompt" not in payload:
        raise OperatorInstructionsParseError(
            "tamara operator envelope missing key: prompt",
            tag="design_spec.parsed.missing-key",
        )
    prompt = payload["prompt"]
    if not isinstance(prompt, str):
        raise OperatorInstructionsParseError(
            "tamara operator envelope prompt must be a string",
            tag="design_spec.parsed.wrong-type",
        )
    ...
    if not isinstance(context, dict):
        raise OperatorInstructionsParseError(
            "tamara operator envelope context must be a mapping",
            tag="design_spec.parsed.wrong-type",
        )
    return {
        "tag": "design_spec.parsed.ok",
        "prompt": prompt.strip(),
        "context": context,
    }


def _assemble_design_spec(parsed: dict[str, Any]) -> dict[str, Any]:
    prompt = parsed["prompt"]
    context = parsed["context"]
    theme = str(context.get("theme", "clean"))
    return {
        "brief": prompt,
        "theme": theme,
        "templates": [
        ...
        "style_tokens": {
            "palette": str(context.get("palette", "neutral")),
            "typography": str(context.get("typography", "sans")),
        },
        "operator_steps": [
            "Align prompt to four-template structure.",
            "Assign style tokens and reusable layout rules.",
            "Return deterministic design specification only.",
        ],
    }
...
def _act(state: RunState) -> dict[str, Any]:
    if not state.model_resolution_trail:
        raise RuntimeError("tamara act invoked before plan; resolution trail is empty")
    last_entry = state.model_resolution_trail[-1]
    if last_entry.cache_prefix_hash is None:
        raise RuntimeError("tamara act expected final plan resolution entry with cache_prefix_hash")
...
def _gate_decision(state: RunState) -> dict[str, Any]:
    ...
    interrupt({"gate_id": "tamara-gate-decision"})
    del state
    return {}
...
def build_tamara_graph() -> StateGraph:
    """Build scaffold-conformant graph for tamara."""
    ...
    if present != SCAFFOLD_NODE_IDS:
        ...
        raise RuntimeError(
            f"generated scaffold drift for tamara; missing={missing} extra={extra}"
        )
    return graph
