"""Generated 9-node scaffold graph for specialist kim."""
...
class OperatorInstructionsParseError(RuntimeError):  # noqa: N818
    """Raised when Kim operator-instructions envelope cannot be parsed."""
...
def _decode_envelope_payload(state: RunState) -> dict[str, Any]:
    if state.cache_state is None or not state.cache_state.cache_prefix:
        raise OperatorInstructionsParseError(
            "kim act envelope cache_prefix is missing",
            tag="coursearc.parsed.missing-key",
        )
    try:
        decoded = json.loads(state.cache_state.cache_prefix)
    except json.JSONDecodeError as exc:
        raise OperatorInstructionsParseError(
            f"kim act envelope cache_prefix is not valid JSON: {exc}",
            tag="coursearc.parsed.malformed",
        ) from exc
    if not isinstance(decoded, dict):
        raise OperatorInstructionsParseError(
            "kim act envelope cache_prefix must decode to a mapping",
            tag="coursearc.parsed.wrong-type",
        )
    return decoded
...
def _parse_operator_instructions_envelope(payload: dict[str, Any]) -> dict[str, Any]:
    if "prompt" not in payload:
        raise OperatorInstructionsParseError(
            "kim operator envelope missing key: prompt",
            tag="coursearc.parsed.missing-key",
        )
    prompt = payload["prompt"]
    if not isinstance(prompt, str):
        raise OperatorInstructionsParseError(
            "kim operator envelope prompt must be a string",
            tag="coursearc.parsed.wrong-type",
        )
    ...
    if not isinstance(context, dict):
        raise OperatorInstructionsParseError(
            "kim operator envelope context must be a mapping",
            tag="coursearc.parsed.wrong-type",
        )
    return {
        "tag": "coursearc.parsed.ok",
        "prompt": prompt.strip(),
        "context": context,
    }


def _assemble_coursearc_instructions(parsed: dict[str, Any]) -> dict[str, Any]:
    prompt = parsed["prompt"]
    context = parsed["context"]
    prompt_words = [part for part in prompt.split() if part]
    return {
        "brief": prompt,
        "audience": str(context.get("audience", "general")),
        "deliverable": "coursearc_operator_plan",
        "steps": [
            "Collect source constraints from payload context.",
            "Build outline with objective, assessment, and pacing anchors.",
            "Return only deterministic operator instructions.",
        ],
...
def _act(state: RunState) -> dict[str, Any]:
    if not state.model_resolution_trail:
        raise RuntimeError("kim act invoked before plan; resolution trail is empty")
    last_entry = state.model_resolution_trail[-1]
    if last_entry.cache_prefix_hash is None:
        raise RuntimeError("kim act expected final plan resolution entry with cache_prefix_hash")
...
def _gate_decision(state: RunState) -> dict[str, Any]:
    ...
    interrupt({"gate_id": "kim-gate-decision"})
    del state
    return {}
...
def build_kim_graph() -> StateGraph:
    """Build scaffold-conformant graph for kim."""
    ...
    if present != SCAFFOLD_NODE_IDS:
        ...
        raise RuntimeError(
            f"generated scaffold drift for kim; missing={missing} extra={extra}"
        )
    return graph
