...
VisualDetailLoad = Literal["low", "medium", "high"]
...
class GarySlideOutput(_StrictModel):
    ...
    file_path: str = Field(min_length=1, description="Local downloaded PNG path.")
...
class SegmentManifestSegment(_StrictModel):
    ...
    visual_file: str = Field(min_length=1)
    ...
    cluster_position: Literal["establish", "tension", "develop", "resolve"] | None = None
...
class IrenePass2AuthoringEnvelope(_StrictModel):
    ...
    procedural_rules: tuple[ProceduralRule, ...] = (
        "behavioral_intent_parity",
        "bridge_cadence",
        "cluster_arc_continuity",
        "motion_perception_confirmation",
    ...
    @classmethod
    def _generated_at_must_be_tz_aware(cls, value: datetime) -> datetime:
