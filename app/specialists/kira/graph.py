...
from app.specialists.kira import _act as _kira_act_impl
from app.specialists._scaffold.contract import SCAFFOLD_NODE_IDS

REPO_ROOT = Path(__file__).resolve().parents[3]
SANCTUM_DIR = _kira_act_impl.SANCTUM_DIR
...
def _act(state: RunState) -> dict[str, Any]:
    return _kira_act_impl.act(state)
...
def _emit_spans(state: RunState) -> dict[str, Any]:
    return specialist_summary_writer.emit_summary_for_state("kira", state, gate_id="G2F")
