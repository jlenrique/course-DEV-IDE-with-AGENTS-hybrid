"""Generated 9-node scaffold graph for specialist aria."""
...
class OperatorInstructionsParseError(RuntimeError):  # noqa: N818
    """Raised when Aria operator-instructions envelope cannot be parsed."""
...
def _decode_envelope_payload(state: RunState) -> dict[str, Any]:
    if state.cache_state is None or not state.cache_state.cache_prefix:
        raise OperatorInstructionsParseError(
            "aria act envelope cache_prefix is missing",
            tag="build_spec.parsed.missing-key",
        )
    try:
        decoded = json.loads(state.cache_state.cache_prefix)
    except json.JSONDecodeError as exc:
        raise OperatorInstructionsParseError(
            f"aria act envelope cache_prefix is not valid JSON: {exc}",
            tag="build_spec.parsed.malformed",
        ) from exc
    if not isinstance(decoded, dict):
        raise OperatorInstructionsParseError(
            "aria act envelope cache_prefix must decode to a mapping",
            tag="build_spec.parsed.wrong-type",
        )
    return decoded
...
def _parse_operator_instructions_envelope(payload: dict[str, Any]) -> dict[str, Any]:
    if "prompt" not in payload:
        raise OperatorInstructionsParseError(
            "aria operator envelope missing key: prompt",
            tag="build_spec.parsed.missing-key",
        )
    prompt = payload["prompt"]
    if not isinstance(prompt, str):
        raise OperatorInstructionsParseError(
            "aria operator envelope prompt must be a string",
            tag="build_spec.parsed.wrong-type",
        )
    ...
    if not isinstance(context, dict):
        raise OperatorInstructionsParseError(
            "aria operator envelope context must be a mapping",
            tag="build_spec.parsed.wrong-type",
        )
    return {
        "tag": "build_spec.parsed.ok",
        "prompt": prompt.strip(),
        "context": context,
    }


def _assemble_build_spec(parsed: dict[str, Any]) -> dict[str, Any]:
    prompt = parsed["prompt"]
    context = parsed["context"]
    return {
        "brief": prompt,
        "module_name": str(context.get("module_name", "lesson_module")),
        "format": "articulate_build_spec",
        "required_sections": [
            "overview",
            "slides",
            "knowledge_checks",
        ...
        "operator_steps": [
            "Normalize prompt scope.",
            "Map scope to section-level build instructions.",
            "Return deterministic build specification only.",
        ],
    }
...
def _act(state: RunState) -> dict[str, Any]:
    if not state.model_resolution_trail:
        raise RuntimeError("aria act invoked before plan; resolution trail is empty")
    last_entry = state.model_resolution_trail[-1]
    if last_entry.cache_prefix_hash is None:
        raise RuntimeError("aria act expected final plan resolution entry with cache_prefix_hash")
...
def _gate_decision(state: RunState) -> dict[str, Any]:
    ...
    interrupt({"gate_id": "aria-gate-decision"})
    del state
    return {}
...
def build_aria_graph() -> StateGraph:
    """Build scaffold-conformant graph for aria."""
    ...
    if present != SCAFFOLD_NODE_IDS:
        ...
        raise RuntimeError(
            f"generated scaffold drift for aria; missing={missing} extra={extra}"
        )
    return graph
