...
from app.specialists.vision import _act as _vision_act_impl
from app.specialists.vision.payload_contract import CONSUMED_PAYLOAD_KEYS
...
def _act(state: RunState) -> dict[str, Any]:
    return _vision_act_impl.act(state)
...
def _emit_spans(state: RunState) -> dict[str, Any]:
    return specialist_summary_writer.emit_summary_for_state("vision", state)


def _gate_decision(state: RunState) -> dict[str, Any]:
    _ = _resume_from_verdict
    interrupt({"gate_id": "vision-gate-decision"})
    del state
    return {}
...
def build_vision_graph() -> StateGraph:
    ...
    if present != SCAFFOLD_NODE_IDS:
        ...
        raise RuntimeError(f"generated scaffold drift for vision; missing={missing} extra={extra}")
    return graph


__all__ = ["CONSUMED_PAYLOAD_KEYS", "TRANSITIONS", "_act", "build_vision_graph"]
