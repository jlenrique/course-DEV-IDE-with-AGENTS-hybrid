"""Reverse-shim: legacy `marcus.facade` -> `app.marcus.facade` (pre-Trial-3 S2 collapse 2026-05-07).

This module is a thin re-export shim. Production content lives at `app.marcus.facade`.
...
Mirror pattern: re-exports the full `app.marcus.facade` namespace (public + select non-`__all__`
attributes that backward-compat callers depend on, e.g., test-only helpers).
"""

from app.marcus.facade import *  # noqa: F401, F403

import app.marcus.facade as _src  # noqa: E402
import sys as _sys  # noqa: E402

# Mirror everything (except dunder attrs) so `from marcus.facade import X` works
